...
\begin{document}
  ...
  \chapter{Introduction}
    ...
    \section{Problem statement}
      ...
      If developers want to build modular applications by composing the application by smaller components (elements, scripts, styles, etc.) \gls{media queries} are no longer applicable.
      ...
      Some problems with implementing element queries natively in \gls{CSS} are:
      \begin{itemize}
    ...
    \section{Objective}
      ...
      In addition, research will be done about the problems of implementing element queries natively, to get a deeper understanding of how an official \gls{API} could look like.
      ...
      \begin{itemize}
        ...
        \item If a custom \gls{API} is developed, how would one make third-party modules (that uses \gls{media queries}) work without demanding a rewrite of all third-party modules? \todo{Remove me?}
    ...
    \section{Significance}
      ...
      Unfortunately, it is currently impossible to create \gls{self-contained} \gls{responsive} modules since conditional styles cannot be applied to elements by element sizes.
      ...
      Modules should be \gls{self-contained} (i.e. they work by themselves) and may not require the user to perform the module logic.
      Another option would be to make the modules context-aware so they can style themselves according to the \gls{viewport}, but then they would not be reusable (since they assume a specific context).
      ...
      \gls{W3C} receives requests and questions about it, but the answer seems to be that no solution will be provided in the near moment. \todo{"near moment". Have I written this somewhere else?}
      An organization called \gls{RICG} have started an initial planning regarding element queries.
      However, things are moving slow and a draft about element queries use cases is still being written.

      Solving this problem would be a big advancement to \gls{web} development, enabling developers to create truly modular components.
    ...
    \section{Delimitations}
      ...
      \item Theory about layout engines, \gls{CSS}, \gls{HTML}, and \gls{responsive} \gls{web} design will be given to fully understand the problem. \todo{Modular theory?}
      \end{itemize}

      \subsection{What will not be done}
      \begin{itemize}
      \item No efforts will be made to solve the problems accompanied with a native solution.
      \item No \gls{API} or similar will be designed for a native solution.
  ...
  \part{Background}\label{part:background}
    ...
    \chapter{Web development}
      ...
      \section{Responsive web design}\label{sec:rwd}
        ...
        By using \gls{media queries}, elements can be styled in the following manner:
  ...
  \part{Theory}\label{part:theory}
    \chapter{Layout engines}
      ...
      \begin{enumerate}
        ...
        \item Layout the elements of the \gls{render tree}.
        \item Render the elements of the \gls{render tree}.
      \end{enumerate}
      ...
      \section{Constructing DOM trees}\label{sec:dom-tree}
        ...
        It should be noted that although \gls{DOM} nodes do include a \code{style} property, the nodes in the \gls{DOM} tree are not affected by the \gls{CSS} cascading, and the style properties do not represent the final style of the element.
      ...
      \section{Render trees}\label{sec:render-tree}
        ...
        In contrast to the \gls{DOM} tree, the \gls{render tree} only contains of elements that will be rendered.
        The nodes of the \gls{render tree} are called \emph{renderers} (also known as \emph{frames} or \emph{render objects}), as the elements are responsible for their own and all subnodes layout and rendering.
        ...
        It should be noted that elements with the \code{visibility} property set to \code{hidden} will be present in the \gls{render tree}, although they are not visible, since they still affect the layout flow.
      ...
      \section{Style computation}\label{sec:style-computation}
        \begin{metatext}
          Both the \gls{render tree} and scripts need to be able to get the final style of an element.
        ...
        \begin{enumerate}
          ...
          \item In inline \gls{CSS} in the \code{style} element attribute.
          \item In scripts modifying the element style through the \gls{DOM} tree.
        ...
        Selector matching conceptually finds all elements in the \gls{DOM} tree that matches a \gls{CSS} selector, to apply the rule set.
        ...
        Inline \gls{CSS} does not cascade since all rules given are automatically matched with the element of the style attribute.
        However, inline \gls{CSS} is considered with highest possible specificity when performing the style cascade.
        It is important to note that the styling methods number five and six are conceptually the same, since they both alter the style property of the \gls{DOM} element.
        The last item is deprecated, and all styles applied this way will be weighted as low as possible.

        If the cascade results in no value for an element style property, then the property can \emph{inherit} a value or have the initial value defined by the \gls{CSS} \emph{property definition}.
        ...
        \begin{table}[ht]\center
          ...
          \end{tabular}
          \caption{The \gls{CSS} property definition format that describes how all element style properties behave.}
      ...
      \section{Parallelization}\label{sec:parallel}
        ...
        For the \gls{web} to be a true competitor to heavy native applications, the page performance needs to be increased (not only the overall browser performance).
    ...
    \chapter{Element queries}
      ...
      Recall from section~\ref{sec:rwd} that \gls{responsive} \gls{web} design is the concept of having elements respond to the given size.
      ...
      Since the only way of applying conditional rules to elements today is by using \gls{media queries}, all \gls{responsive} styles must be defined by the application and not by the modules (because only the application knows how much space each module will be given in the layout).
      ...
      Although the size of the container element is typically the most interesting element query with \gls{responsive} \gls{web} design, element queries as a concept does not need to be limited to that.
      It is also possible to apply conditional rules by any element display property, color, text alignment, and so on.
      Theoretically it should be possible to query any style property of an element, but the focus will be the \code{width} and \code{height} properties since they are the most interesting in \gls{responsive} \gls{web} design.
      ...
      \section{Problems}
        \begin{metatext}
          ...
          It is important to understand the problems in order to speculate about a potential native \gls{API} design.
        ...
        \subsection{Performance}
          ...
          Further, it is common for internal \glspl{API} in layout engines to request updated element styles that do not require layout to resolve (non-layout properties such as \code{color} and \code{font-family}).
          Since a layout of the element query selector target is required in order to resolve the correct element style, such internal \glspl{API} requires a layout in order to obtain the correct element style (even for non-layout related properties).
...
#foo:eq(min-width: 500px) #child { 
  ...
  font-size: 1em;
      ...
      \section{Possible solutions}
        ...
        \subsection{Viewport elements}\label{sec:viewport-elements}
          \begin{metatext}
            ...
            This approach has been discussed at \gls{W3C} and \gls{RICG} assumes that this is a prerequisite to a native implementation.
            \todo{cite where this has been gathered from?}
          \end{metatext}

          By limiting element queries to special \gls{viewport} container elements that can only be queried by child elements, much of the problems are resolved.
          ...
          The size of \glspl{viewport} is not dependent on their children, and children may only target the closest ancestor \gls{viewport} element in the element queries.
          This way, cyclic rules can no longer occur since the children may not alter the \gls{viewport} size.
          The reason that a new \gls{HTML} tag is proposed instead of a new \gls{CSS} property that defines the behavior is because the layout engine in the latter case needs to resolve the styles for all elements in order to know which are \gls{viewport} elements.
          With a new \gls{HTML} tag, the layout engine can detect that that the element is a \gls{viewport} before any resolving styles.
          \todo{Write why it is important to know \gls{viewport} elements before resolving styles.}

          This approach also solves many of the performance problems, but not all.
          The internal \glspl{API} that request non-layout information for the elements using element queries only need to make sure that the containing \gls{viewport} element has been laid out before resolving the styles (which is a much better situation that the unrestricted version of element queries).
          Further, the layout can be done in one pass as long as the \gls{viewport} elements are laid out before the children.
          It is still a bit inconvenient that the layout engine would need to evaluate all element query selectors in the middle of a layout pass (after the that the \gls{viewport} elements has been laid out) in order to resolve the styles for the \gls{viewport} children.
          Parallelization is also easier, since the queries may only be targeting the closest \gls{viewport} element.
          ...
          It should be noted that \code{iframe} elements are not suitable as an alternative to the proposed \code{viewport} element, since \code{iframe} elements are much more limited by nature (creates a new document and script context).

          Obviously, this limits element queries a lot.
          The fact that the size of the \gls{viewport} element cannot depend on the children (like normal block elements do), limits the usability.
          ...
          This is a powerful insight, since the general use case would be to write element queries against the width and have the elements adapt their heights accordingly (including the \gls{viewport} element).
      ...
      \section{Native API design}
        \begin{metatext}
          In this section a possible native element query \gls{API} will be presented.
        ...
        As presented in subsection~\ref{sec:viewport-elements}, \gls{viewport} elements solve many of the element query problems and are therefore assumed to be a part of a native \gls{API}.
  ...
  \part{Third-party framework}\label{part:framework}
    ...
    \chapter{Framework design}
      ...
      \section{Technical goals}\label{sec:techincal-goals}
        ...
        For instance, the requirement~\ref{itm:req_prolyfill} is beneficial to satisfy with a plugin since the \gls{API} for native element queries can only be guessed at this point of time, which will probably lead to frequent changes to the plugin.
      ...
      \section{Architecture}\label{sec:arcithecture}
        ...
        \end{itemize}
        Some of the subsystems may be partially accessible through the public \gls{API} such as the plugin handler and the element resize detector.
    ...
    \chapter{Implementation}
      \section{Element resize detection}\label{sec:imp_erd}
        ...
        \end{metatext}
          ...
          Only frames (elements that render it's content independently of the container element, such as \code{iframe} and \code{object}) support the resize event by standard since they have their own \gls{viewport}.
          ...
          Since the framework needs to support all commonly used \gls{HTML} elements a solution to listen to any element resize needs to be found.
          ...
          \end{itemize}
          The first solution is appealing because it does not mutate the \gls{DOM} and it works on all browsers since it does not rely on special element behaviors or similar.
          However, in order to prevent the \gls{responsive} elements lagging behind the size changes of the user interface, the polls need to be performed quite frequently.
          ...
          The third solution does mutate the \gls{DOM} and relies on special element behavior, but it offers many advantages.
          ...
          \begin{itemize}
            \item \textbf{\gls{CSS} selectors may break}:
              Since target elements get an extra child (the frame element), \gls{CSS} selectors may behave differently.
