...
\begin{document}
  ...
  \chapter{Introduction}
    ...
    \todo[inline,color=red]{Tomas: Missing a proper guidenance (and outline) of my work vs summaries of other work. Which parts should expert web developers read?}
    ...
    \section{Objective}
      ...
      \begin{itemize}
        ...
        \item
          ...
          Likewise, if the \gls{API} requires a special \gls{element} handling process, it could be hard to use with popular frameworks. \todo{Should I give examples like Angular React Backbone Bootstrap?}
  ...
  \part{Background}\label{part:background}
    ...
    \chapter{Browsers} 
      ...
      \todo[inline,color=red]{Tomas: This is not about browsers. Maybe change the title of the chapter, or change content so it reflects the title.}
    ...
    \chapter{Web development}
      ...
      \section{From documents to applications}
        ...
        \todo[color=red]{Tomas: Remove recently and add the date instead.}
        ...
        \todo[inline,color=red]{Tomas: Maybe write about the difficulties in supporting the standards (that the was/is fragmented in supporting features)}
  ...
  body {
    background-color: yellow;
      ...
      \section{Modularity}\label{sec:modularity}
        ...
        However, since modulas are allowed to be nested (i.e., modules may depend on submodules that may depend on other submodules and so on) and that the logic responsibility is moved from the application to the module, the complexity of managing them is added.
  ...
  \part{Theory}\label{part:theory}
    \chapter{Layout engines}
      ...
      \section{Constructing DOM trees}\label{sec:dom-tree}
        ...
        Instead, for scripts to obtain the final computed style for an \gls{element} a special function needs to be called (e.g. \code{getComputedStyle} in \gls{JavaScript} \cite{w3c_cssdom_getcomputedstyle}).
        \todo{Should the reference to getComputedStyle really be cited like that?}
  ...
  <body>
    ...
    <script>
      ...
      \section{Render trees}\label{sec:render-tree}
        ...
        Some properties depend on the flow of the \gls{document}, and need to be computed through a layout.
      ...
      \section{Parallelization}\label{sec:parallel}
        ...
        \todo[color=green]{Marcos: Reference to work-stealing system?}
        \todo{Reference http://supertech.csail.mit.edu/papers/steal.pdf?}
      ...
      \section{Layout thrashing and how to avoid it}
        \begin{metatext}
          This section will present what \gls{layout thrashing} is and how it can be prevented.
          Avoiding \gls{layout thrashing} is important when optimizing \gls{DOM} manipulations, and will be a key element to constructing a performant third-party framework.
        \end{metatext}
...
doubleWidths(elements); // ~7 ms
        ...
        It should be noted that both versions recalculate styles for the same number of elements.
        ...
        The style calculation was reduced from \textasciitilde40 ms to \textasciitilde2 ms because the first version calculated the style of each element one at a time and therefore suffered from $nO_{s}$ in overhead, where $n$ is the number of elements.
        Since the second version enabled the layout engine to calculate the style of all elements in a batch, the overhead was reduced to only $O_{s}$.
        However, it should be noted that both versions have time complexity $O(n)$.
        
        The major part of time reduction is due to the layout being significantly reduced.
        The reason why the layout was significantly reduced is because the two version do not perform layout on the same amount of elements.
        ...
        So the minimum number of nodes that will be need to be laid out is given by $n + n_{a}$, where $n_a$ is the number of ancestor elements.
        ...
        \todo[inline]{Have a T(n) function for style computations aswell?}
        ...
        \todo[inline]{Refer to some section in the appendix for how to read the timeline images?}
    \chapter{Element queries}\label{chp:eq}
      ...
      \todo[color=green]{Marcos: Maybe say, "One solution to this is element queries". It might turn out that there is some other solution.}
      \todo{Waiting for response from Marcos regarding this.}
      ...
      \section{Problems}
        \begin{metatext}
          ...
          \todo[color=green]{Marcos: You would need a citation here. Also, I don't think the above is true. We (browsers) haven't seen that much demand for this until recently... and even still, it's not super high on the list of priorities.}
          \todo{Waiting for response}
        ...
        \subsection{Performance}
          ...
          \todo[color=green]{Marcos: sizes -> dimensions}
          \todo{Waiting for response}
  ...
  font-size: 2em;
      ...
      \section{Possible solutions}\label{sec:solutions}
        \begin{metatext}
          ...
          The second approach is to provide element queries as a third-party framework which operates element queries on top of the layout engine (in JavaScript).
...
#outer:eq(width > 500px) #foo { ... }
        ...
        Another approach to enable element queries for developers is to provide a \gls{third-party} \gls{JavaScript} framework.
        Since a \gls{JavaScript} framework do not depend on vendors of \glspl{layout engine} (other than features in the current language specifications) it can therefore be designed in any desired way.
        It would be more feasible to have unrestricted element queries in a \gls{JavaScript} framework, than implementing it natively.
        Since the element queries feature would be operated by the framework, \glspl{layout engine} can be parallelized unhindered and independently of how the framework handles the queries.
        Similar, cycle detection and handling can be handled on top of the \gls{layout engine} by the framework.
        Although an implementation in \gls{JavaScript} will be slower than a \gls{native} implementation, it can still be beneficial to have such feature implemented as a \gls{third-party} framework to avoid complex and restricting code in \glspl{layout engine}.
...
%         \todo{Continue this if possible?}
  \part{Third-party framework}\label{part:framework}
    ...
    \chapter{Analysis of approaches}\label{ch:current_approaches}
      ...
      \todo{Not really happy with the dynamic vs static. Need to define this exactly. Is it dynamic because CSS is parsed at runtime or does it need a way to rerun the framework on new elements as well?}
      \begin{table}[ht]\center
        \tiny
      ...
      % \cite{eq_imp_sickles}               Valid syntax.   Semi-automatic. Part of bootstrap-like framework. No JS API, instead parses CSS.
...
elq.start(jqueryElementsCollection);
        ...
        \subsection{Plugin API}
          ...
          \begin{itemize}
            ...
            \item \textbf{Batch processor}: via the \code{elq.createBatchProcessor} property that refers to a function that creates a \glslink{batch processing}{batch processor} intance.
          ...
          Instead of having a single \glslink{batch processing}{batch processor} instance shared among all framework entities (i.e., core subsystems and plugins), each entity has to create an own instance.
...
%           \end{lstlisting}
        ...
        \subsection{Plugins}
          ...
          It should be noted that although not written in the examples, the framework and all plugins support all combinations of custom attribute declarations so if desired, the framework is able to conform to the \gls{HTML} standard.
