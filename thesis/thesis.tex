...
\begin{document}
  ...
  \chapter{Introduction}
    ...
    \section{Problem statement}\label{sec:problem}
      ...
      See the included \gls{document} in Section~\ref{sec:problem-formulation} of the appendix for a more practical problem formulation.
    ...
    \section{Outline}
      ...
      Section~\ref{sec:layout-engines} briefly describes how \glspl{layout engine} operates and how \gls{JavaScript} can be optimized by avoiding \gls{layout thrashing}.
      The section ends with presenting the current research of parallelizing \glspl{layout engine}, which is important to understand since element queries affects the parallelization of \glspl{layout engine}.
      ...
      The chapter also gives insight into the current research of element queries and presents some difficulties that the W3C and are facing with implementing element queries in \glspl{layout engine}.
      Section~\ref{sec:eq-problems} describes the problems that element queries implies to \glspl{layout engine} in detail and Section~\ref{sec:eq-approaches} presents some approaches discussed by the W3C to overcome some of the problems.

      \chref{ch:library} presents the \gls{third-party} element queries \gls{JavaScript} library that enables developers to use element queries today.
      ...
      \chref{ch:eval} presents the empirical evaluation of the library, which includes altering the popular style framework \gls{Bootstrap} and measuring the performance of subsystems in different \glspl{browser}.
      The \gls{Bootstrap} framework provides many \gls{responsive} \gls{CSS} classes to make user interfaces \gls{responsive} by using \gls{media queries}.
  ...
  \chapter{Background}\label{ch:background}
    This chapter aims to present sufficient background to understand why element queries is a desired feature and why it is hard to implement \glslink{native}{natively} in \glspl{browser}.
    ...
    \section{Layout engines}\label{sec:layout-engines}
      ...
      Section~\ref{sec:layout-engine-reference} to Section~\ref{sec:layout-process} describes briefly how \glspl{layout engine} operates and also presents key \gls{layout engine} concepts.
  ...
  \chapter{Element queries}\label{ch:eq}
    ...
    Section~\ref{sec:eq-problems} presents some of the issues of implementing element queries \glslink{native}{natively} in \glspl{layout engine}.
  ...
  \chapter{The element queries library}\label{ch:library}
    ...
    \section{Details of subsystems}\label{sec:library-imp-details}
      ...
      \subsection{Batch processor}\label{sec:imp_batch_processor}
        ...
        The \glslink{batch processing}{batch processor} is the foundation for good performace of the library, and is therefore used by several subsystems.
        ...
        The automated asynchronous processing of \glslink{batch processing}{batches} is important because some subsystems do not know how many operations are to be performed when called.
        ...
        However, if the \code{start} method is called mutliple times synchronously \gls{layout thrashing} occurs since one \glslink{batch processing}{batch} for each method call is created.
        ...
        By delaying the \glslink{batch processing}{batch} to execute asynchronously, on he next \gls{JavaScript} event tick in most \glspl{layout engine}, all synchronous calls to the method is grouped into the pending \glslink{batch processing}{batch}.
        ...
        Processing \glslink{batch processing}{batches} in different levels is important when multiple operations that interfer with each other needs to be grouped together in a \glslink{batch processing}{batch}.
        For instance, a function that is to double an \gls{element}'s width and read the new height benefits by being \glslink{batch processing}{batch} processed in three levels: reading the width, mutating the width, and reading the height of an \gls{element}.
        ...
        See listing~\ref{code:batch-processor-example-2} for an example implementation of such function that uses a leveled \glslink{batch processing}{batch processor}.
      ...
      \subsection{Element resize detection}\label{sec:imp_erd}
        ...
        The rewritten version (from now on referred to as the \gls{ELQ} scroll-based solution) is performs the algorithm steps in the following levels using the \glslink{batch processing}{batch processor}:
        \begin{enumerate}
        ...
        However, it is still beneifical to \glslink{batch processing}{batch} process the algorithm for such \glspl{layout engine} since only pure layouts need to be performed (instead of having to recompute styles and synchronize the \gls{DOM} and \glspl{render tree} before each layout).        
  ...
  \chapter{Related work}\label{ch:related-work}
    There are numerous \gls{third-party} element queries libraries, which this chapter aims to list and analyze.
    Since many of them share the same characteristics, they are classified and analyzed in groups by characteristics.

    Identified \gls{third-party} libraries that directly or indirectly enables element queries are presented in table~\ref{table:approaches-classifications} along with a classification of each library.
    ...
    While not directly offering element queries, the library enables the possibility to conditionally style elements by element criterion and thus makes it a good candidate to using \gls{CSS} and element queries.
  ...
  \chapter{Discussion and Conclusions}\label{ch:discussion-conclusions}
    ...
    Therefore, the goal of this thesis was to develop a \gls{third-party} \gls{JavaScript} library that enables developers to use element queries.
    ...
    Such good performance was achieved by \gls{batch processing} all \gls{DOM} interaction, by using the leveled \glslink{batch processing}{batch processor} subsystem that has been developed to be used by \gls{ELQ}.
    ...
    \section{Future work}
      ...
      As of now, different subsystems of \gls{ELQ} are forced to create their own \glslink{batch processing}{batch} processors, which is favorable in some cases.
      However, it can also be favorable to have a shared instance \glslink{batch processing}{batch} processor in order to avoid unnecessary layouts between subsystems.
      For instance, the element resize detection subsystem and the \code{elq-breakpoints} plugin both uses a \glslink{batch processing}{batch} processor to avoid \gls{layout thrashing}.
      ...
      This could in theory be solved by sharing a \glslink{batch processing}{batch} processor between them so that both executes read, write and force operations in sync with each other.
