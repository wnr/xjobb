...
\begin{document}
  ...
  \chapter{Background}\label{ch:background}
    ...
    \section{Layout engines}\label{sec:layout-engines}
      ...
      \subsection{Layout thrashing and how to avoid it}\label{sec:layout-thrashing}
        ...
        \end{lstlisting}
        Now, the \code{doubleWidths} function performs two batches that both iterates over the \glspl{element} collection.
        In the first batch, all widths of all \glspl{element} are acquired and stored in a collection.
        ...
        In the second batch, all \glspl{element} are assigned the new widths based on the widths retrieved in the first batch.
        ...
        Let $C_{s}$ denote the time it takes for the layout engine to calculate the style of one \gls{element}, then the total time needed for style computations in the first version is given by $T(n) = n(O_s + C_s)$.
        Since the second version enabled the layout engine to calculate the style of all \glspl{element} in a batch, the total time needed was reduced to $T(n) = nC_s + O_s$.
        ...
        Since the first version forces the layout engine to perform a layout in each iteration, the minimum number of \glspl{element} laid out is given by $n(n + n_a)$.
        Similar to style computation, performing a layout also has an overhead cost $O_{l}$.
        Let $C_{l}$ denote the time it takes for the layout engine to layout one \gls{element}, then the total time needed for layout in the first version is given by $T(n) = n(O_{l} + C_{l}(n + n_a))$.
        The second version enables the layout engine to batch process the \gls{DOM} manipulations, and therefore only one layout of the subtree will need to be performed, which results in the minimum number of \glspl{element} laid out being $n + n_a$.
  ...
  \chapter{Element queries}\label{ch:eq}
    ...
    \section{Definitions and Usages}\label{sec:eq-definitions}
      ...
      It can somewhat simplified be described as that \gls{media queries} target the document root and up (i.e., the \gls{viewport}, browser, OS, device, input mechanisms, etc.) while element queries target the document content (i.e., \glspl{element} of the document).
    ...
    \section{Approaches to overcome the problems}\label{sec:eq-approaches}
      ...
      By limiting element queries to a special separated \gls{viewport} container \glspl{element} (i.e., a sub-\gls{viewport} of the document) that can only be queried by child \glspl{element}, many of the problems are resolved \cite{w3c_eq_mail,ricg_irc_log,ricg_issue_viewport}.
  ...
  \chapter{The element queries library}\label{ch:library}
    ...
    \section{API design}\label{sec:elq-api}
      ...
      \subsection{Plugins}
        ...
        \subsubsection{Mirror}\label{sec:plugin-mirror}
          ...
          By mirroring element breakpoint classes, \glspl{element} can be styled by criterion of any element (ancestor, sibling, child, etc.).
    ...
    \section{Details of subsystems}\label{sec:library-imp-details}
      ...
      \subsection{Batch processor}\label{sec:imp_batch_processor}
        ...
        As part of the initialization, the element resize detection subsystem may be called so it can prepare \glspl{element} to be detectable.
        ...
        For instance, a function that is to double an \gls{element}'s width and read the new height benefits by being batch processed in three levels: reading the width, mutating the width, and reading the height of an \gls{element}.
        ...
        \begin{lstlisting}[gobble=10,label={code:batch-processor-example-2},caption={Example function that doubles an \gls{element}'s width and returns the new height with a callback. The function uses the leveled batch processor to avoid layout thrashing and thus gains a 45-fold speedup.},captionpos=b]]
      ...
      \subsection{Element resize detection}\label{sec:imp_erd}
        ...
        Only \glspl{document} emit resize events and therefore resize events can only be observed for frame \glspl{element} (since a frame \gls{element} has its own document).
        ...
        \begin{enumerate}
          \item\label{itm:erd-approach-polling}
            ...
            This approach is the most robust, as it supports all \glspl{element} (including void \glspl{element}) and provides excellent browser compatibility.
        ...
        For detecting resize events it performs excellent, but injecting many \code{object} \glspl{element} is quite a heavy task for browsers and it consumes a significant amount of memory as later presented in Section~\ref{sec:eval-perf-erd}.
        ...
        \begin{enumerate}
          ...
          \item                                       The algorithm waits for the \code{load} event handler to be called by the layout engine. When the handler is called, a \code{resize} event handler is attached to the document of the \code{object} \gls{element}.
        \end{enumerate}
        ...
        By observing the \code{style} attribute of an \gls{element} and the content of it (if the \gls{element} size depends on its children), it is possible to detect direct style changes to an \gls{element} (i.g., changes to the style attribute by JavaScript).
        The main limitation is that it is not possible to observe the computed style state of an \gls{element}.
        For instance, detecting that the width of an \gls{element} has been set to 50 \% with JavaScript is possible (it would not have been possible to detect the style change if the width style was calculated by a CSS cascade).
        ...
        This could theoretically be resolved by also observing the parent of the \gls{element}, and so on up to the root of the document.
  ...
  \chapter{Empirical Evaluation}\label{ch:eval}
    ...
    \section{ELQ Bootstrap}\label{sec:eval-bootstrap}
      The \gls{responsive} parts of Bootstrap 3.3.2 (the grid, containers, \gls{responsive} utility classes, forms, etc.) are built with \gls{media queries}, and therefore only behave as desired when the Bootstrap \glspl{element} can use the full width of the \gls{viewport}.
      ...
      This implies that modules that uses any \gls{responsive} Bootstrap classes inherently only behave as desired when the module can use the full width of the \gls{viewport}, which means that such modules are not \gls{encapsulated} and therefore many of the benefits of modules disappear.
      ...
      \begin{figure}[htbp]
        ...
        \caption{
          ...
          The pag only behaves as desired if the \gls{viewport} width is large enough so that the width of each column is bigger than the largest \glslink{media queries}{media query} breakpoint, as shown in the right figure.
      ...
      Two Bootstrap classes have been chosen to be treated as \glspl{viewport} (that will be \code{elq-breakpoints} \glspl{element}): \code{container} and \code{container-fluid}.
      ...
      Listing~\ref{code:bootstrap-init-js} presents an example of such JavaScript code that dynamically adds the required element attributes and initializes all \gls{responsive} \glspl{element}.
      \begin{lstlisting}[gobble=8,label={code:bootstrap-init-js},caption={Example of JavaScript code that adds the required \gls{ELQ} attributes dynamically to all \gls{responsive} \glspl{element} and initializes them.},captionpos=b]]
      ...
      As shown in the figures, the double column documentation page using the original Bootstrap styles the two columns as if they both had the whole \gls{viewport} width available.
      Therefore the two columns intersects each other because some content \glspl{element} are styled wider than a column.
      The double column documentation page using \gls{ELQ} Bootstrap on the other hand enables all \gls{responsive} \glspl{element} to style themselves according a parent container \gls{element}.
      ...
      The visual result is the same as having two documentation sites of the original Bootstrap in two \code{iframe} \glspl{element} as two columns (since \code{iframe} \glspl{element} creates a separate \gls{viewport}).
      ...
      \begin{figure}[hp!]
        ...
        \end{minipage}
        \caption{
          The left image shows the original Bootstrap header, which is broken since the content of both columns are styled as if they had the full \gls{viewport} width.
    ...
    \section{Performance}\label{sec:eval-performance}\label{sec:eval-perf-erd}
      ...
      %\subsection{Element resize detection}\label{sec:eval-perf-erd}
        ...
        \todo{Write that the scroll-based solution probably has a bug that makes it perform so bad? In only perfoms bad when expanding the \gls{element}, not shrinking it... This however seem to be a bug with Safari, not the implementation.}
        \begin{table}[ht]\center
          \tiny
          ...
          \end{tabular}
          \caption{Performance of the two solutions in different layout engines when operating on 100 \glspl{element}.}
  ...
  \chapter{Related work}\label{ch:related-work}
    ...
    \todo{Not really happy with the dynamic vs static. Need to define this exactly. Is it dynamic because CSS is parsed at runtime or does it need a way to rerun the library on new \glspl{element} as well?}
    \begin{table}[ht]\center
      \tiny
    ...
    \todo{Write why again?}
    Library \cite{eq_imp_gss} evaluates all \glspl{element} on \gls{DOM} mutations, using the \code{MutationObserver} \gls{API} as discussed in Section~\ref{sec:imp_erd}, which does not detect all element resize events.
  ...
  \chapter{Discussion and Conclusions}\label{ch:discussion-conclusions}
    ...
    Since no syntax or standard has been specified by the \gls{W3C} yet for element queries, the syntax and usage can be formed in any way as long as it solves the problem of conditionally styling \glspl{element} by element criterion.
    ...
    Unrestricted element queries (i.e., not restricting target \glspl{element} to special \gls{viewport} \glspl{element}) imposes limitations to layout engines, and are therefore infeasible to implement.
    ...
    Further, element queries requires the layout engine to perform a layout in order to be able to get the final size of an \gls{element}.
    ...
    ELQ and its plugins provides a low level API for building \gls{responsive} design frameworks and applications by enablind the user to conditionally style \glspl{element} with valid CSS.
    ...
    \section{Future work}
      ...
      However, it is uncertain if the \gls{viewport} size is the only layout factor to static pages as browser and end-used styles might affect pages in a way that would imply that a layout change is desired without affecting the \gls{viewport} size.
      ...
      This way, all \glspl{element} would have the right classes before a layout is performed by the layout engine and thus resulting in an instant valid page layout.
      ...
      \section{Full-scale figures}
        \subsection{Bootstrap}
          ...
          \begin{figure}[htbp!]
            ...
            \caption{Double column documentation page powered by Bootstrap with showing the header section of the page. The \gls{viewport} is big enough to give each column sufficient space.}
