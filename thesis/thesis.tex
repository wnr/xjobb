  ...
  \chapter{Background}\label{ch:background}
    ...
    \section{Web development}\label{sec:web-dev}
      ...
      \subsection{From documents to applications}\label{sec:doc-to-app}
        ...
        Examples of things that were being done with \gls{DHTML} are: refreshing pages for the user so that new content is loaded, giving feedback on user input without involving the server, and animating content.
  ...
  \chapter{The element queries library}\label{ch:library}
    ...
    \section{Details of subsystems}\label{sec:library-imp-details}
      ...
      \subsection{Batch processor}\label{sec:imp_batch_processor}
        ...
        For instance, a function that doubles an \gls{element}'s width and reads the new calculated height benefits by being \glslink{batch processing}{batch} processed in three levels: reading the width, mutating the width, and reading the new height.
        ...
        The function achieves a 45-fold speedup when applied to 1000 \glspl{element} by processing the \glslink{batch processing}{batch} in levels compared to not doing so.
