...
\usepackage[backend=biber,style=numeric,defernumbers=true,sorting=nty]{biblatex}
\usepackage{multirow}
\usepackage{hyperref}
\hypersetup{
}
...
\DeclareBibliographyCategory{relatedlibraries}
\addtocategory{relatedlibraries}{eq_imp_localised-css,eq_imp_breakpointsjs,eq_imp_mediaclass,eq_imp_elementquery,eq_imp_element-queries,eq_imp_responsive-elements,eq_imp_magichtml,eq_imp_sickles,eq_imp_css-element-queries,eq_imp_responsive-elements-2,eq_imp_selector_queries,eq_imp_eqjs,eq_imp_breaks2000,eq_imp_classquery,eq_imp_eqcss,eq_imp_gss,eq_imp_prollyfill-min-width}
\DeclareBibliographyCategory{theses}
\addtocategory{theses}{mjelde2014performance}
\DeclareBibliographyCategory{w3cmaterial}
\addtocategory{w3cmaterial}{w3c_dom,w3c_dom_4,w3c_dom2_events,w3c_css_style_attr,w3c_visuren,w3c_css_mq,w3c_css_selectors,ricg_draft,w3c_www,ricg_site,w3c_eq_mail,ricg_irc_log,ricg_issue_viewport}

% \renewcommand*{\glstextformat}[1]{\textsf{#1}}
\defglsdisplayfirst[\glsdefaulttype]{\textit{#1}}

\title{ELQ: Extensible Element Queries for Modular Responsive Web Components}
\foreigntitle{ELQ: Utbyggbara Element Queries för Modulära Responsiva Webbkomponenter}
...
\begin{document}
  ...
  \begin{abstract}
    ...
    Additionally, implementing element queries imposes circularity and performance problems, which need to be resolved before writing a standard.
    ...
    In order to enable developers to create encapsulated responsive modules, while waiting for native support, a third-party element queries JavaScript library named ELQ has been developed.
  ...
  \begin{foreignabstract}{swedish}
    Responsiv webbutveckling är ett populärt sätt att stödja enheter med varierande egenskaper (storlek av visninsområdet, inmatningsmekanismer, mediumtyper, etc.) genom att ange olika stilar för ett dokument beroende på enhetens egenskaper med hjälp av CSS \emph{media queries}.
    ...
    För att möjliggöra skapandet av enkapsulerade responsiva moduler, i väntan på webbläsarstöd, har ett tredjepartsbibliotek för element queries namngett ELQ skapats i JavaScript.
  ...
  \section*{Acknowledgements}
    I would like to thank my supervisors Tomas Ekholm, Philipp Haller and Stefan Sennerö for their enthusiastic involvement in this thesis.
  ...
  \chapter{Introduction}
    ...
    \section{Targeted Audience}
      This thesis is targeted for \gls{web} developers that wish to gain a deeper understanding of element queries and how they can be used without \gls{native} support by using the \gls{ELQ} library.
      Heavy use of \gls{web} terminology is being used and intermediate \gls{web} development knowledge is beneficial.

      For readers that are not familiar with \gls{web} terminology there is a glossary at page~\pageref{main} of this thesis.
      Unfamiliar readers might also want to notice the distinction between the words ``user'' and ``end-user''.
      For instance, an application is a user of modules while the human interacting with the application is an end-user.
    \section{Problem Statement}\label{sec:problem}
      ...
      It should be noted that no effort of implementing a \gls{native} solution has been made.
    ...
    \section{Objective}
      ...
      To do so it is necessary to be aware of the premises, such as \gls{browser} limitations and specifications that need to be conformed.
      ...
      \begin{itemize}
        ...
        \item How can \gls{element} size changes be observed without any \gls{native} support?
        \item
          How can a custom \gls{API} be crafted that enables element queries and still conform to the language specifications? Is it possible to create an \gls{API} that feels natural to \gls{web} developers and works in tandem with other tools and libraries?
          For instance, if the \gls{API} requires custom \gls{CSS} syntax then \gls{CSS} preprocessors, linters and validators cannot be used.
    ...
    \section{Significance}
      ...
      Without element queries, \gls{responsive} modules force the user to style them properly depending on the \gls{viewport} size, which defeats the purpose of modules.
      ...
      Clearly, no sound option exists for having \gls{responsive} modules so solving this problem would be a big advancement to \gls{web} development.
      ...
      The \gls{W3C} receives requests and questions about it, and the \gls{RICG} has started writing a draft about element queries use cases \cite{ricg_draft}.
      This thesis may be helpful for the \gls{W3C} and others to get an overview of the problem and possibly get ideas how subproblems can be handled.

      With the developed element queries library presented in this thesis, developers are enabled to create truly \gls{responsive} modules while waiting for the \gls{W3C} to standardize a solution.
      The novelty of the library is that it is performant, compatible, flexible, powerful, and extensible.
      No other related library simultaneously provides all of these desirable properties.
      The library provides a unique way to overcome some limitations of \gls{CSS} without requiring currently-invalid syntax or parsing of an extended, incompatible version of \gls{CSS}.
      Related approaches that overcome similar limitations require invalid syntax, which brings many drawbacks (e.g., reduced performance and development tools incompatibility).
      It has also been shown that the library has significantly better performance than related approaches in most configurations.
      Further, the \gls{API} of the library makes it easy to integrate into existing projects, which has been shown by altering the \gls{Bootstrap} framework to use element queries instead of \gls{media queries} by only altering \textasciitilde50 lines of style code.
      The library is able to provide such positive properties while still maintaining compatibility with legacy browsers (down to Internet Explorer version 8).
    ...
    \section{Delimitation}
      ...
      All theoretical studies and work have been performed to support the development of the library.
      The problems of implementing element queries \glslink{native}{natively} are addressed, but no effort has been made to solve the problems.
      Also, no \gls{API} or similar has been designed for a \gls{native} solution.
      Graphical design of end-user interfaces are not addressed, other than necessary for understanding the problem.
    \section{Choice of Methodology}
      ...
      The library design is motivated by identified requirements of element queries, which have been gathered partly by the literature study and studies of existing responsive applications.
      The decision to write the non-\gls{native} library in \gls{JavaScript} was made, because \gls{JavaScript} is the common scripting language of \glspl{browser}.
      For increased compatibility and ease of use, the use of other languages compiling to \gls{JavaScript} was avoided.

      The performance and browser compatibility of the library have been evaluated by empirically testing the library in different browsers.
      Since there are numerous related libraries only the performance of the element resizing detection subsystem of the libraries has been evaluated.
      There are only three different approaches to such a subsystem, and therefore the work needed to compare the libraries was reduced significantly.
      This restriction is motivated by the fact that the element resizing detection subsystem performs the heaviest task by far, and therefore has the largest performance impact.
      Another restriction is that only fully functioning automated related libraries have been evaluated, since others cannot be considered an equivalent alternative to the developed library.
      ...
      It is hard to objectively evaluate features and \gls{API} designs, but some objective data were gathered by the evaluation.
      ...
      The premises of the library development method are considered valid since a \gls{JavaScript} library is the natural choice when creating a browser extension that is to be used by a large audience of developers.
      The premises of the \gls{Bootstrap} altering method are considered valid since \gls{Bootstrap} has the properties that are required for such an evaluation to be sound (e.g., it is widely used in production, has a large code base, and has sufficiently advanced responsive features).
      The premises of the performance evaluation method are considered valid since it is possible to evaluate different systems in the same configuration in order to achieve comparable results.
    ...
    \section{Outline}
      ...
      Section~\ref{sec:layout-engines} briefly describes how \glspl{layout engine} operate and how avoiding \gls{layout thrashing} can increase the performance of \gls{JavaScript} code.
      The section ends with presenting the current research about parallelizing \glspl{layout engine}, which is important to understand since element queries affect the parallelization of \glspl{layout engine}.
      ...
      The chapter also gives insight into the current research about element queries and presents some difficulties that the W3C is facing with implementing element queries in \glspl{layout engine}.
      ...
      The library design and \glspl{API} are presented along with two plugins and their \glspl{API}.
      Last, some subsystems of the library are presented in detail including a highly optimized solution to observing element resize events using a leveled \glslink{batch processing}{batch processor} subsystem.
      ...
      \chref{ch:related-work} describes related libraries and presents advantages as well as drawbacks to the different approaches.
      ...
      \chref{ch:discussion} discusses advantages and drawbacks of non-\gls{native} solutions, and related libraries are compared to the developed library.
      The fulfillment of the identified requirements of the developed library are also discussed.
      
      \chref{ch:conclusions} summarizes the conclusions of this thesis, and shows how the formulated problem is solved and how the scientific question is answered.

      \chref{ch:future-work} presents possible future work, which includes many ideas of extensions and further improvements to the library.
  ...
  \chapter{Background}\label{ch:background}
    ...
    Section~\ref{sec:web-dev} presents a brief history of \gls{web} development and motivates why \gls{responsive} \gls{web} design and modular development are both popular and needed concepts in modern \gls{web} development.
    Ultimately, Section~\ref{sec:rwd-modular-problem} describes the problem of having modular \gls{responsive} modules, which element queries solves.    
    ...
    As will be shown later in Chapter~\ref{ch:eq}, element queries as a concept limits the parallelization of \glspl{layout engine}.
    Section~\ref{sec:layout-thrashing} defines \gls{layout thrashing} and shows how it can be avoided in order to improve performance.
    ...
    \section{Web Development}\label{sec:web-dev}
      ...
      Section~\ref{sec:modularity} similarly defines and motivates modular development, which also is needed to understand the problem that element queries solve.
      ...
      This section is the key motivation to why element queries are needed.

      \subsection{From Documents to Applications}\label{sec:doc-to-app}
        Since \gls{web} development trends are not easily pinned to exact dates, this section only presents dates as guidance and should not be regarded as exact dates for the events.
        ...
        As further described in Section~\ref{sec:www} in the appendix, \glspl{browser} were initially applications that displayed \gls{hypertext} \glspl{document} with the ability to fetch linked \glspl{document} in a user-friendly way.
        ...
        Different stylesheet languages were being developed to enable the possibility of separating styling from the content.
        In 1996 the \gls{W3C} officially recommended \gls{CSS}, which came to be the preferred way of styling \gls{web} content.
        Since \gls{HTML} is only a markup language it is not possible to generate dynamic content, which was sufficient at the time \gls{HTML} was used only for annotating links in research \glspl{document}.
        ...
        However, when the content was delivered to the client (\gls{browser}) it was still static.
        There was no way for the server to change the content that the client had received, unless the client requested another \gls{document}.
        ...
        The \gls{JavaScript} language came to be the common client-side scripting language of the \gls{web}.
        Examples of things that were being done with \gls{DHTML} are: refreshing pages so that new content was loaded, giving feedback on user input without involving the server, and animating content.
        Plugins also started to exist during this time, which enabled \glspl{browser} to execute embedded programs.
        ...
        With the increase of smart devices (such as phones, televisions, cars, game consoles, etc.) that include \glspl{browser} with limited \gls{third-party} runtimes, plugins quickly decreased in popularity.
        ...
        This enabled the \gls{AJAX} \gls{web} development technique, which became a popular way of communicating with servers ``in the background'' of the page.
        ...
        In a meeting held by the \gls{W3C} in 2004, it was proposed to extend \gls{HTML} to ease the creation of \gls{web} applications that was rejected.
        The \gls{W3C} was criticized for not listening to the need of the industry, and some members of the \gls{W3C} left to create the \gls{WHATWG}.
        The \gls{WHATWG} started working on specifications to ease the development of \gls{web} applications that were later grouped together under the name \gls{HTML5}.
        ...
        \gls{HTML5} is an evolutionary improvement process of \gls{HTML}, which means that \glspl{browser} are adding support as parts of the specification are finished.
      ...
      \subsection{Responsive Web Design}\label{sec:rwd}
        ...
        Since typically only desktop computers with monitors accessed websites they were designed for a minimum \gls{viewport} size.
        If the size of the \gls{viewport} was smaller than the supported size, the site would look broken.
        ...
        When a \gls{browser} requests a resource, a \emph{\glslink{browser}{user agent} string} is usually sent with the request to identify what kind of \gls{browser} the user is using. \todo{Explain user agent, that all \glspl{browser} are user agents but not the other way around.}
        By reading the \glslink{browser}{user agent} string on the server side, a mobile-friendly version of the site could be served if the user was sending mobile \glslink{browser}{user agent} strings and a desktop version could be served otherwise.
        The mobile version would be designed for a maximum width, and the desktop for a minimum width.
        ...
        Second, this approach does not scale well with new devices entering the market.
        \todo[color=green]{Marcos: Not only devices (also \gls{browser}).}
        For instance, tablets are somewhere in the middle of mobiles and laptops in size, which would require another special version of the site.
        Further, when desktops support touch actions and smartphones support mouse actions, even more versions of the website need to be developed in order to satisfy all user devices.
        Third, the desktop site assumes that desktop users have big screens (which usually is true).
        However, there is no guarantee that the \gls{browser} \gls{viewport} is big just because the screen is big.
        ...
        See listing~\ref{code:mq-example} for an example of how \gls{media queries} can be used to conditionally style \glspl{element} with relation to the \gls{viewport} size.
      ...
      \subsection{Modularity}\label{sec:modularity}
        ...
        By creating modules that can be used in any context with well-defined responsibilities and dependencies, developing applications is reduced to the task of simply configuring modules (to some extent) to work together to form a bigger application.
        ...
        A big challenge to software development is to write software that is reliable (i.e., should not have bugs) and easy to change.
        ...
        The following is a quote from Rich Hickey:
        \begin{quote}
        ...
        \end{quote}
        By separating the software into well-defined parts (i.e., modules) where each part has a single responsibility and ideally performs a single task, complexity can be reduced.
        ...
        Positive side effects come with modularity.
        ...
        The very nature of modules not being context aware enable them to be reused in other projects (or other parts of the same application).
        Reusability is not only important to speed up the development process of new software projects; it also eases the management of a large code base.
        ...
        However, since modules are allowed to be nested (i.e., modules may depend on submodules that may depend on other submodules and so on) and the logic responsibility is moved from the application to the module, the complexity of managing them is added.
      ...
      \subsection{The Problem of Responsive Modules}\label{sec:rwd-modular-problem}
        ...
        Therefore, it is desired to extend the \gls{responsive} \gls{web} design concept to also let modules respond to their own size.
        Since the only way to conditionally style \glspl{element} is by using \gls{media queries}, all \gls{responsive} styles must be defined by the application and not by the modules (because only the application knows how much space each module is given in the application layout).
        ...
        So developers today have three choices: writing modules that are not \gls{responsive} and \gls{encapsulated}, or writing modules that are \gls{responsive} and not \gls{encapsulated} (leaving the styling to the user), or simply not writing responsive modules at all.
        ...
        See figure~\ref{fig:problem-eq} for the desired behavior of the module instances of the example page.
        ...
        As evident in the figure, the page is broken since either all instances are colored blue when the \gls{viewport} is wide or red when it is narrow.
        ...
        \begin{figure}[ht]
          ...
          \begin{minipage}{.5\textwidth}
            \centering
            \includegraphics[width=0.7\linewidth]{images/mq-small}
          \end{minipage}
          \caption{
            ...
            In the left image, the \gls{viewport} is wide and therefore all instances are colored blue.
            In the right image, the \gls{viewport} is narrow and therefore all instances are colored red.
          }
        ...
        Since around 2011 many articles have been written about the problem and how badly \gls{web} developers need element queries \cite{eq_article_localised-css,eq_article_backalley,eq_article_mqhack,eq_article_tabatkjr,eq_article_filament,eq_article_tyson,eq_article_neal,eq_article_css-tricks,eq_article_hugo,eq_article_fremycompany,eq_article_discource,eq_article_matt}.
        ...
        The \gls{W3C} receives requests and questions about it, and the \gls{RICG} has started writing a draft \cite{ricg_draft} about element queries use cases.
    ...
    \section{Layout Engines}\label{sec:layout-engines}
      ...
      Only the \gls{layout engine} of the \gls{browser} is of importance, since element queries do not affect any other \gls{browser} subsystem.
      This section gives a brief explanation of the layout process that the engines generally perform, along with some usual optimizations that are performed to speed up the process.
      The render process is not described, as it is not relevant for element queries.
      ...
      The role of the \gls{layout engine} and a reference architecture for \glspl{browser} is also presented.
      ...
      Section~\ref{sec:layout-thrashing} defines \gls{layout thrashing} and presents how it can be avoided.
      Section~\ref{sec:parallel} gives insight into the current state of the research about parallelizing the layout process.
      ...
      \subsection{Reference Architecture}\label{sec:layout-engine-reference}
        ...
        It should be noted that although \gls{Blink} is a recent \gls{fork} of \gls{WebKit}, they do differ quite significantly as shown in Chapter~\ref{ch:eval}.
        Due to \gls{Trident} being closed source, only \gls{WebKit}, \gls{Blink}, and \gls{Gecko} is considered in this chapter.
        ...
        \begin{table}[ht]\center
          ...
          \caption{
            ...
            See Section~\ref{sec:layout_engines_market_share} for more information about how the market share data were gathered.}
      ...
      \subsection{Constructing DOM Trees}\label{sec:dom-tree}
        The \gls{DOM} defines a platform-neutral model for events and node trees, which is used for representing and interacting with \glspl{document} \cite{w3c_dom,w3c_html}.
        ...
        \Glspl{layout engine} construct \gls{DOM} trees by parsing \gls{HTML} with any included \gls{CSS} and \gls{JavaScript} \cite{garsiel2011browsers,w3c_css21}.
        ...
        If such script mutates the \gls{DOM}, then the parser needs to evaluate the changes made by the script and update the \gls{DOM} tree.
        ...
        Instead, for scripts to obtain the final computed style for an \gls{element} special functions need to be called (e.g. \code{getComputedStyle} in \gls{JavaScript}\footnote{See \url{http://dev.w3.org/csswg/cssom/\#dom-window-getcomputedstyleelt-pseudoelt} for details about the function.}).
        ...
        \begin{lstlisting}[gobble=10,caption={Simple example of reentrant HTML. The \gls{layout engine} needs to reconstruct the \gls{DOM} tree after executing the \gls{JavaScript}. The \gls{layout engine} can not form the final layout of the page until all \gls{JavaScript} has been executed.}, captionpos=b, label={code:reentrant-html}]
          <html>
            <body>
              <div id="container">
      ...
      \subsection{Render Trees}\label{sec:render-tree}
        ...
        In order to know how to render them the final style of each renderer need to be computed, which is done by the \gls{layout engine} while constructing the tree.
        ...
        Typically nodes of the \gls{DOM} tree have a \code{1:1} relation to nodes of the \gls{render tree}, but the relation can also be \code{m:n}.
        Since the \gls{render tree} only contains nodes that affect the rendered result, nodes that do not affect the layout flow of the \gls{document} and that are not visible are not be present in the \gls{render tree}.
        For instance, a \gls{DOM} node with the \code{display} property set to \code{none} has the relation \code{1:0} (it is not be present in the \gls{render tree} because it is not visible and does not affect the layout flow).
        It should be noted that nodes with the \code{visibility} style property set to \code{hidden} are present in the \gls{render tree}, although they are not visible, since they still affect the layout flow.
        ...
        This is because some properties depend on the flow of the \gls{document}, which cannot be resolved by style cascading and instead need to be computed through a layout.
        The layout process resolves the final position and size of all renderers.
      ...
      \subsection{Style Computation}\label{sec:style-computation}
        ...
        \gls{CSS} property definitions is also described.
        ...
        \begin{enumerate}
          ...
          \item In internal \gls{CSS} \code{style} tags in the \gls{document}.
        ...
        \todo{Clarify this.}
        The four first items have in common that they are cascading their style rules through selector matching, and may define rule sets.
        ...
        The rule sets are weighted so that if a property is assigned values by multiple rule sets the one with highest weight is applied.
        ...
        \end{itemize}
        The weighting process continues by calculating the \gls{specificity} of all rule set selectors, as the weight is increased by high \gls{specificity}.
        This makes rule sets with more specific selectors override rule sets with more general selectors.
        Finally, if two rule declarations have the same weight, origin and \gls{specificity} the rule set specified last wins.
        ...
        For instance, the \code{width} property definition states that legal values are absolute lengths, percentages, and \code{auto} (which lets the \gls{browser} decide).
        The percentages are relative to the containing block.
        The initial value is \code{auto} and the properties apply to all non-replaced inline \glspl{element}, table rows, and row groups.
        ...
        If a property may inherit values, it inherits the value of the first ancestor that has a value that is not \code{inherited}.

        This process resolves most style properties, but as stated in Section~\ref{sec:render-tree} some properties require a layout in order to be resolved.

        \begin{table}[ht]\center
          \tiny
          \begin{tabular}[t]{ r | l }
            \textbf{Value} & Defines the legal values and the syntax. \\
            \textbf{Initial} & The initial value of the property. \\
      ...
      \subsection{The Layout Process}\label{sec:layout-process}
        ...
        The layout process decides the final computed style of all \glspl{element}, and needs to be done before rendering.
        ...
        This is possible because the geometry of \glspl{element} typically do not depend on siblings or children\todo{Seems weird to say that flow is not dependent on children, since the height is propagated upwards.}.
        Layout is performed recursively by starting at the root of the tree, let the renderer render itself and all of its children which render themselves and their children and so on.
        ...
        The system marks the renderers in the tree that need a layout, which avoids layout of unaffected \glspl{element}.
        ...
        Because the \gls{API} of \code{getComputedStyle} promises resolved values for all style properties, calling the function forces a layout of the affected subtree (flushing the incremental layout commands queue).
        ...
        \end{enumerate}
        Of course, only the children that need a layout are affected (dirty, or global layout) in step 2.
        ...
        The widths are calculated by the \code{width} style of the \glspl{element}, relative to the containing \gls{element} width.
        Margins and paddings are also taken into account when calculating the widths.
        When the width of an \gls{element} is calculated, it needs to be controlled against the \code{min-width} and \code{max-width} style properties to make sure that the width is inside the given range.
        If the content of an \gls{element} does not fit to the calculated width the \gls{element} needs to break up the content into multiple renderers in order to expand the height (text usually needs to perform line break when the width is too small).
        A renderer that has decided that it needs to break the content up into multiple renderers propagates to the parent renderer that it needs to perform the breaking.
      ...
      \subsection{Layout Thrashing}\label{sec:layout-thrashing}
        ...
        In that function, the computed style of the element is requested, which forces a style computation for that \gls{element}.
        If any \gls{DOM} node affecting the element is marked as dirty, a layout also needs to be performed before returning the computed style of the \gls{element}.
        When the computed style has been acquired, the width of the \gls{element} is set to a new value, which is a \gls{DOM} mutation that needs to be synchronized with the \gls{render tree}.
        Therefore, the element and its ancestors are marked as dirty.
        The next time \code{getComputedStyle} is called, which happens when processing the next element in the collection, a layout is forced since \gls{DOM} nodes that affect the \gls{element} have been marked as dirty.
        ...
        \begin{figure}[ht]
          \centering
          \includegraphics[scale=0.6]{images/layout-thrashing-example-1-big}
          \includegraphics[scale=0.6]{images/layout-thrashing-example-1}
          \caption{Two timeline images of executing the example code given in listing~\ref{code:thrashing-example}. Both images show the same timeline, where the top image shows the whole execution and the bottom only shows a small section of the execution. Notice that they do not share the same time axis. Here it is clear that \gls{layout thrashing} occurs, since layout is being done repeatedly throughout the whole execution (each iteration in the loop).}
        ...
        While it is algorithmically inefficient to iterate over the same collection twice --- the \glslink{batch processing}{batched} approach achieves a \textasciitilde100-fold speedup compared to the original version.
        \begin{lstlisting}[gobble=10,caption={Example of avoiding \gls{layout thrashing} by \gls{batch processing} read and write operations to the \gls{DOM}. The code reads and doubles the widths of 1000 \glspl{element} in \textasciitilde7 ms.}, captionpos=b, label={code:thrashing-example-fixed}]
          /* Doubles the width of each element in the elements collection parameter. */
          function doubleWidths(elements) {
            var widths = [];
        ...
        In the first \glslink{batch processing}{batch}, the width of all \glspl{element} are acquired and stored in a collection.
        ...
        Since the script does not read the \gls{DOM} in any way in the second \glslink{batch processing}{batch}, all \gls{DOM} mutations can be queued by the \gls{layout engine} to be \glslink{batch processing}{batch} processed.
        ...
        When an element and its ancestors have been marked as dirty, a layout needs to be performed on the subtree that contains at least all $n$ \glspl{element} and possibly some ancestor \glspl{element}.
        ...
        The second version enables the \gls{layout engine} to \glslink{batch processing}{batch process} the \gls{DOM} manipulations, and therefore only one layout of the subtree needs to be performed, which results in the minimum number of \glspl{element} laid out being $n + n_a$.
      ...
      \subsection{Parallelization}\label{sec:parallel}
        No longer can performance of an application increase over time without any code changes (as opposed to the times when clock speeds of the \glspl{CPU} increased rapidly) \cite{parallelizing_the_web_browser}.
        ...
        In this section a small summary is given about the current research front, and how the parallelization of \glspl{layout engine} can be approached.
  ...
  \chapter{Element Queries}\label{ch:eq}
    Now that a good understanding of \glspl{browser} (especially the \gls{layout engine}), \gls{responsive} design, and modular development has been acquired it is time to address element queries --- the solution to the modular \gls{responsive} \gls{web} design problem as presented in Section~\ref{sec:rwd-modular-problem}.
    ...
    It also presents the identified general use case for element queries and describes some terminology.
    Since no syntax has been defined yet for element queries, a pseudo-syntax is presented in this section to be used throughout this chapter.
    ...
    \section{Definitions and Usages}\label{sec:eq-definitions}
      ...
      The main difference is the type of criteria that can be used; in \gls{media queries} device, document, and media criteria are used, while element criteria are used in element queries.
      ...
      Element queries also define an element \emph{style state}, that reflects in which state the target element of an element query is in (often relative to the breakpoints).
      ...
      As already stated, element queries are not yet standardized and the exact behavior and syntax are still undefined.
      In this thesis, the pseudo-syntax of element queries is defined as a pseudo-class named \code{eq}.
      Recall from the \gls{CSS} selectors level 3 specification \cite{w3c_css_selectors} that pseudo-classes permit selection based on extended element information, and is often described as performing selection by the state of an \gls{element}.
      ...
      This implies that element queries that target void \glspl{element} (i.e., \glspl{element} that cannot contain content) are also not crucial, and hence are not regarded as a general use case.
    ...
    \section{Problems}\label{sec:eq-problems}
      ...
      \subsection{Performance}
        ...
        If element queries that rely on the size of \glspl{element} are present, the following process needs to happen:
        \begin{enumerate}
        ...
        Unfortunately, this means that if there is any matching element query selector at page load, at least two layout passes need to be performed.
        ...
        Since a layout of the element query selector target is required in order to resolve the correct \gls{element} style, such internal \glspl{API} would need to force a layout to obtain the correct \gls{element} style (even for non-layout related properties).
        ...
        Element queries limit the parallelization, as \glspl{layout engine} would not be able to layout subtrees in parallel since an \gls{element} in one subtree might affect an \gls{element} in another subtree.
      ...
      \subsection{Circularity}\label{sec:cyclic-rules}
        ...
        \Glspl{browser} could do such analysis during parsetime in order to warn about or handle the cycles.
        ...
        However, in another scenario the inherited font size might not be big enough to make the \code{\#foo} element wider than 300 pixels and therefore the rules are not cyclic.
    ...
    \section{Approaches to Overcome the Problems}\label{sec:eq-approaches}
      ...
      In order to avoid cyclic rules, the following limitations to the \gls{viewport} element are defined:
      \begin{enumerate}
        \item\label{viewport-rule-1} The size of the \gls{viewport} element may not be dependent on its children. This implies that code that causes a \gls{viewport} \gls{element} to fit its content is invalid.
        \item\label{viewport-rule-2} A selector may only include the \gls{viewport} \gls{element} targeted by element queries as part of the expression of a selector (not the right-most simple selector).
      ...
      However, it is still inconvenient that the \gls{layout engine} needs to evaluate all element query selectors in the middle of a layout pass (after that the \gls{viewport} \glspl{element} have been laid out) in order to resolve the styles for the \gls{viewport} children.
  ...
  \chapter{The Element Queries Library}\label{ch:library}
    ...
    This section is the main motivation to that the library should be plugin based.
    ...
    This section presents the main outcome of this thesis, as it shows how element queries are exactly realized by the library plugins.
    ...
    This section also describes optimizations that have been done.
    ...
    \section{Technical Goals}\label{sec:techincal-goals}
      ...
      \begin{enumerate}
        \item \textbf{Features}
          \begin{enumerate}
            \item\label{itm:req_resize_detect}    The library needs to augment \gls{native} element queries in the sense that styles are applied automatically when \glspl{element} resize.
            \item\label{itm:req_flexible}         The features should be flexible and not limited to the general use case.
      ...
      Plugin-based libraries solve this problem by providing plugins so that developers can compose their own custom tailored solutions for their specific use cases.
      ...
      For instance, the requirement~\ref{itm:req_prolyfill} is beneficial to satisfy with a plugin since the \gls{API} of \gls{native} element queries can only be guessed at this point of time, which will probably lead to frequent changes to the plugin.
    ...
    \section{Architecture}\label{sec:arcithecture}
      ...
      If a plugin needs to write a custom version of a subsystem in the core in order to work, the subsystem is just an extra payload to be loaded.
      Each subsystem added to the core impacts the performance negatively (by slower load time in the best case) for all users, so they must impose a real value to existing and future plugins.
      ...
      This way the more advanced users can choose which subsystems to alter, while keeping the library easy to use for the general use case described in Section~\ref{sec:eq-definitions}.
      ...
      \begin{itemize}
        \item \textbf{Reporter:}
          ...
          Other options such as throwing exceptions on errors or warnings could also be set at a global library level.
          ...
          To avoid code duplication it is also beneficial to have a centralized reporting system so plugins do not need to perform the same compatibility checks (such as checking if the \gls{browser} actually supports console\footnote{See \url{https://developer.mozilla.org/en-US/docs/Web/API/Console} for more information.} reporting).
        ...
        \item \textbf{Cycle detector:}
          ...
          When a plugin wishes to update the size state of an \gls{element} (which in turn applies the conditional styles) it can ask this system if the update seems to be part of a cycle.
      ...
      It should be noted that some subsystems have been intentionally left out due to being too small or insignificant to list in this section.
    ...
    \section{API Design}\label{sec:elq-api}
      \subsection{Public API}\label{sec:public-api}
        ...
        The plugin identifier is either the plugin definition, instance, or name.
        ...
        When called, it invokes the \code{start} methods on all registered plugins that give them the opportunity to initiate the \glspl{element} according to their own mechanisms.
        To satisfy the requirements \ref{itm:natural} and \ref{itm:assumption} the function is agnostic about the \glspl{element} collection --- all objects that are iterable and contain \glspl{element} are accepted.
      ...
      \subsection{Plugin API}\label{sec:plugin-api}
        ...
        Since the plugin handler and element resize detector subsystems are exposed through the public \gls{API} they cannot be directly accessed through the plugin \gls{API}.
        ...
        This method facilitates interplugin communications.
        ...
        \begin{itemize}
          \item 
            ...
            The default behavior of the reporter is to report to the \gls{browser} console if possible.
          \item
            ...
            A third update time parameter, which defines when the update was requested, is optional and defaults to the time of the method call.
          \item 
            ...
            Two important options of the \code{createBatchProcessor} are the \code{async} and \code{auto} options.
      ...
      \subsection{Plugins}
        ...
        The plugins presented here are the suggested solution to element queries according to the research of this thesis.
        ...
        \subsubsection{Breakpoints}\label{sec:plugin-breakpoints}
          This plugin is named \code{elq-breakpoints} and it observes element resize events in order to update the size states of \glspl{element} according to defined breakpoints.
          It does not have any dependencies other than the element resize detector of the \gls{ELQ} core.
          ...
          The attribute \code{elq-breakpoints\-width="300 500"} defines that the \gls{element} should have width breakpoints at 300 and 500 pixels.
          Height breakpoints are defined in the same way with the attribute \code{elq-breakpoints-height}.
          The possible size states of the \gls{element} would then be (in pixels):
          \begin{itemize}
            \item $width < 300$
            \item $300 \leq width < 500$
            \item $500 \leq width$
          \end{itemize}
        ...
        \subsubsection{Mirror}\label{sec:plugin-mirror}
          ...
          Therefore, the structure of the application is not known when writing the \gls{CSS}, so no assumptions can be made about the markup structure.
          Consider the markup given in listing~\ref{code:elq-mirror-example} for an example \gls{HTML} structure.
          ...
          As of the \gls{CSS} selectors level 3 specification, it is not possible to select the nearest ancestor of an element \cite{w3c_css_selectors}.
          ...
          \end{lstlisting}
          Notice that the breakpoint classes are now used in conjunction with the paragraph simple selector instead of the container in the CSS selectors.
          This way, the conditional styles behave as expected since all paragraphs are styled by the nearest (instead of any) container \gls{element}.
          ...
          The plugin is at the moment restricted to the nearest ancestor \code{elq-breakpoint} \gls{element}, but could easily be extended to support more advanced mirroring constellations.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%% Implementation
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \pagebreak
    \section{Details of Subsystems}\label{sec:library-imp-details}
      ...
      \subsection{Batch Processor}\label{sec:imp_batch_processor}
        ...
        Both Section~\ref{sec:layout-process} and \ref{sec:layout-thrashing} are recommended to read in order to understand the optimizations described in this section.
        ...
        As part of the initialization, the element resizing detection subsystem may be invoked so it can prepare \glspl{element} to be detectable.
        ...
        For instance, a function that doubles an \gls{element}'s width and reads the new calculated height benefits by being \glslink{batch processing}{batch} processed in three levels: reading the width, mutating the width, and reading the height.
        Such function could also benefit from automatically processing the \glslink{batch processing}{batch} so that the user may call the function multiple times synchronously.
        ...
        The function achieves a 45-fold speedup when applied to 1000 \glspl{element} compared to not processing the \glslink{batch processing}{batch} in levels.
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %%%%%%%%%%%%%%%%%%%%%%%%%%%% Element resizing detection
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      \subsection{Element Resizing Detection}\label{sec:imp_erd}
        ...
        \begin{enumerate}
          \item\label{itm:erd-approach-polling}
            ...
            A short polling interval (high frequency) leads to being able to detect resize events quicker but having worse performance.
            A long polling interval (low frequency) leads to not being able to detect resize events as quickly but having better performance.
          ...
          \item\label{itm:erd-approach-object}
            ...
            It has been shown by \cite{backalley} that \code{object} is the most suitable frame \gls{element} to be used for this purpose as they have good \gls{browser} compatibility and are easy to work with.
          ...
          \item\label{itm:erd-approach-scroll}
            ...
            This solution injects an \gls{element} to the target element that contains multiple overflowing \glspl{element} that listen to \code{scroll} events\footnote{See \url{http://www.w3.org/TR/DOM-Level-3-Events/\#event-type-scroll} for more information about the \code{scroll} event.}.
            ...
            A container \gls{element} that contains the four \glspl{element} is injected and styled so that it matches the size of the target \gls{element} and does not affect the page visually.
          \item\label{itm:erd-approach-flow}
            ...
            \gls{Blink} still supports the events but has deprecated them since version 34 \cite{blink_deprecated_flow_events}.
        \end{enumerate}
        ...
        Recall from Section~\ref{sec:layout-process} that \glspl{layout engine} typically keep a layout queue in order to perform layout in \glslink{batch processing}{batches} for increased performance.
        ...
        The object-based, scroll-based and flow-based solutions have similar characteristics and have all been originally presented by \cite{backalley}.
        The flow-based solution was presented first, but was rejected in favor of the scroll-based solution that in turn was rejected in favor of the object-based solution.
        ...
        \begin{itemize}
          ...
          \item \textbf{The target \gls{element} must be positioned}:
            ...
            Since the injected element must be a child of the target \gls{element} (and not moved upwards in the render tree), the target \gls{element} cannot be positioned \code{static} (i.e., the default positioning for many \glspl{element}).
        ...
        \begin{enumerate}
          ...
          \item\label{itm:erd-algo-scroll-level-2}
            ...
            All mutations performed in this level can be queued by \glspl{layout engine}.
        ...
        \gls{Gecko} and \gls{WebKit} are not able to do so and therefore each iteration in step~\ref{itm:erd-algo-original-scroll-5} forces a layout.
        ...
        The \code{MutationObserver} \gls{API} defined in the W3C working draft of the DOM level 4 specification \cite{w3c_dom_4} may seem like a good candidate to use for detecting \gls{element} resize events at first glance.
      ...
      \subsection{Detecting Runtime Cycles}\label{sec:imp_cycle_detector}
        ...
        \begin{enumerate}
          ...
          \item Iterate $L_{states}$ from beginning to end (starting with the most recent tuple).
          \begin{enumerate}
  ...
  \chapter{Empirical Evaluation}\label{ch:eval}
    This chapter presents the empirical evaluation of the \gls{ELQ} library.
    ...
    \gls{Bootstrap} uses \gls{media queries} to provide \gls{responsive} \gls{CSS} classes that are often desired to use in \gls{web} applications.
    By using element queries instead, the classes become \gls{encapsulated} and can be used in \gls{responsive} modules.
    
    Section~\ref{sec:eval-performance} evaluates the performance of \gls{ELQ} and related libraries in different browsers.
    Since there are numerous related libraries only the performance of the element resizing detection subsystem of the libraries has been evaluated.
    This restriction is motivated by the fact that the element resizing detection subsystem performs the heaviest task by far, and therefore has the largest performance impact.
    ...
    \section{ELQ Bootstrap}\label{sec:eval-bootstrap}
      ...
      \begin{figure}[htbp]
        ...
        \caption{
          ...
          Since the columns only fill half of the \gls{viewport} width and are styled as they both fill the whole \gls{viewport} width, the page appears broken as shown in the left image.
          The page only behaves as desired if the \gls{viewport} width is large enough so that the width of each column is bigger than the largest \glslink{media queries}{media query} breakpoint, as shown in the right image.
          See figure~\ref{fig:appendix-bootstrap-mq-header-small} and~\ref{fig:appendix-bootstrap-mq-header-big} in the appendix for full scale images.}
      ...
      Most changes were similar to the changes shown in listing~\ref{code:bootstrap-less-breakpoints-usage}, which basically replaces the \glslink{media queries}{media query} syntax with the \gls{ELQ} element queries syntax.
      ...
      The double column documentation page using \gls{ELQ} \gls{Bootstrap} on the other hand enables all \gls{responsive} \glspl{element} to style themselves according to a parent container \gls{element}.
      ...
      \begin{figure}[hp!]
        ...
        \end{minipage}
        \caption{
          The left image shows the original \gls{Bootstrap} header, which is broken since both columns are styled as if they have the full \gls{viewport} width.
          ...
          See figure~\ref{fig:appendix-bootstrap-mq-header-small} and~\ref{fig:appendix-bootstrap-eq-header-small} in the appendix for full scale images.}
      ...
      \begin{figure}[htb!]
        ...
        \caption{
          ...
          Here it is clear that both columns are styled as if each has the full \gls{viewport} width available, as they display the large utility classes in green.
          The right image shows same section of the same page, using \gls{ELQ} \gls{Bootstrap}.
          ...
          See figure~\ref{fig:appendix-bootstrap-mq-matrix-small} and~\ref{fig:appendix-bootstrap-eq-matrix-small} in the appendix for full scale images.
        }
      ...
      \begin{figure}[htb!]
        ...
        \caption{
          ...
          The right image shows the same section of the same page, using \gls{ELQ} \gls{Bootstrap}.
          The grids detect that only half of the \gls{viewport} width is available, and therefore style themselves accordingly.
          See figure~\ref{fig:appendix-bootstrap-mq-grid-small} and~\ref{fig:appendix-bootstrap-eq-grid-small} in the appendix for full scale images.
        }
    ...
    \section{Performance}\label{sec:eval-performance}\label{sec:eval-perf-erd}
      ...
      %\subsection{Element resizing detection}\label{sec:eval-perf-erd}
        ...
        The solution does not scale well as the number of \glspl{element} increases.
        ...
        \begin{figure}[ht]
          ...
          \begin{center}
            ...
            \begin{minipage}[t]{.5\textwidth}
              ...
              \end{tikzpicture}
            \end{minipage}
          \caption{The injection performance of the scroll-based element resizing detection solution provided by \cite{eq_imp_css-element-queries}. The left graph shows the injection time of the scroll-based solution. The right graph also includes the object-based solution for reference. The heap memory usage graph has been omitted as the memory usage of the scroll-based solutions is too low for reliable measurements.}
  ...
  \chapter{Related work}\label{ch:related-work}
    ...
    A constraint-based approach is presented individually as it differs from the other libraries.

    Identified libraries that directly or indirectly enable element queries are presented in table~\ref{table:approaches-classifications}.
    ...
    The related libraries are classified by the following characteristics:
    \begin{itemize}
      \item \textbf{Syntax}:  Libraries can either require custom syntax or valid syntax.
                              Custom syntax is considered to be invalid syntax since it does not conform to \gls{web} language standards (custom \gls{CSS} is the most common case).
                              Libraries that do not require custom syntax are considered to have valid syntax.
      \item \textbf{Page type}: All libraries support static pages and some also support dynamic pages.
                                ...
                                Dynamic pages may change layout at runtime and therefore need an element queries library runtime with \gls{element} resize detection.
    ...
    \begin{table}[ht]\center
      ...
      \begin{tabular}[t]{ l p{3cm} l l l p{3cm} }
        ...
        \cite{eq_imp_prollyfill-min-width} &  Element Media Queries &                       Invalid &   Non-void \glspl{element} &   Dynamic &   Flow-based element resizing detection. \\
        \cite{eq_imp_localised-css} &         Localised CSS &                               Invalid &   Arbitrary \glspl{element} &  Dynamic &   Polling-based element resizing detection. \\
        ...
        \cite{eq_imp_element-queries} &       Element Queries &                             Valid &     Non-void \glspl{element} &   Dynamic &   Object-based element resizing detection. \\
        \cite{eq_imp_css-element-queries} &   CSS Element Queries &                         Valid &     Non-void \glspl{element} &   Dynamic &   Scroll-based element resizing detection. \\
        \cite{eq_imp_selector_queries} &      Selector queries and responsive containers &  Valid &     Arbitrary \glspl{element} &  Dynamic &   Polling-based element resizing detection. \\
      \end{tabular}
    ...
    Since they do not conform to the \gls{CSS} standard, new features can be supported through the custom \gls{CSS}.
    ...
    The libraries \cite{eq_imp_localised-css,eq_imp_selector_queries} use polling to detect element resize events and therefore support arbitrary element resizing detection.
    However, as presented in Section~\ref{sec:imp_erd}, polling is undesired.
    \todo{Write why again?}

    The three remaining libraries use three different injection solutions, as described in Section~\ref{sec:imp_erd}, to detect element resize events; \cite{eq_imp_element-queries} uses the object-based solution, \cite{eq_imp_prollyfill-min-width} uses the flow-based solution, and \cite{eq_imp_css-element-queries} uses the scroll-based solution.
    According to the evaluation in Section~\ref{sec:eval-performance}, all three libraries are generally less performant than the element resizing detection system used in \gls{ELQ}.
    ...
    As the name suggests, the idea of \gls{CCSS} is to layout \glspl{document} by constraints that would imply a unifying implementation mechanism (i.e., using a constraint solver).
  ...
  \chapter{Discussion}\label{ch:discussion}
    ...
    Different parts of the library have been shown to have different development rates, which also is an argument in favor to having the library plugin-based.
    Two plugins were developed that enable adequately advanced features for the general use case (as described in Section~\ref{sec:eq-definitions}) that satisfies goals \ref{itm:req_bp_js} and \ref{itm:req_conditional_css}.
    ...
    A key restriction was made to the \gls{API} in order to satisfy goal \ref{itm:req_big_rewrite}, \ref{itm:natural}, \ref{itm:req_valid_syntax} and \ref{itm:assumption}: it must conform to the current \gls{web} standard and must therefore not require invalid syntax.
    ...
    This design decision has proved to be valuable when performing the altering of \gls{Bootstrap} as described in Section~\ref{sec:eval-bootstrap}.
    If the plugin would require invalid syntax it would not be as easy to alter the \gls{Bootstrap} \gls{LESS} files, since the \gls{LESS} preprocessor would not understand the syntax.
    ...
    It is guaranteed to detect cycles, as long as plugins register element style state changes to the subsystem.
    \todo{Quickly summarize performance}
    
    \paragraph{Element resizing detection}
    The most difficult goals to achieve were \ref{itm:req_resize_detect} and \ref{itm:req_performance} that require the library to be both automatic (i.e., it should evaluate element queries on element resize events) and performant.
    ...
    No related library conforms to both goals, as related libraries either do not observe element resize events or do so in a non-performant way.
    ...
    At least the solution performs significantly better than the existing solutions used by related libraries.
    ...
    Both the element resize event detection system and the leveled \glslink{batch processing}{batch processor} were decided to be released as two independent open source projects\footnote{See \url{https://github.com/wnr/element-resize-detector} for the element resize event detector and \url{https://github.com/wnr/batch-processor} for the leveled \glslink{batch processing}{batch} processor.}, since they are general enough to be used in other projects than \gls{ELQ}.

    \paragraph{Drawbacks}
    Unfortunately, all \gls{JavaScript} element queries libraries have inherent drawbacks.
    The most imminent drawback is perhaps that element queries are not evaluated when \glspl{layout engine} render the page for the first time, as \glspl{layout engine} usually render a speculative state of the page before all content has been downloaded and parsed (e.g., external scripts).
    \Glspl{browser} behave a bit different regarding the speculative rendering, but at least \gls{Blink}, \gls{Gecko}, and \gls{WebKit} generally render the page before all external scripts have been executed.
    ...
    However, evaluating element queries synchronously in the head of a \gls{document} is impossible, since the \gls{layout engine} has yet to parse and construct the document body \gls{DOM}.
    ...
    So, a flash of invalid style at page load is inevitable (and that is also the case with all related libraries that evaluate element queries at runtime).
    For instance, the \gls{ELQ} \gls{Bootstrap} example documentation page would initially be rendered as it should be on small screens, since the small layout is the default that it falls back to if no queries match.
    This could be avoided by rendering a white rectangle over the \gls{viewport} and remove it when the page has been rendered correctly.
    ...
    It is desired to define the breakpoints in \gls{CSS} (since the purpose of \gls{CSS} is to define the style of a document), as with \gls{media queries}.
    ...
    \gls{ELQ} and its plugins provide a low level \gls{API} for building \gls{responsive} design frameworks and applications by enabling developers to conditionally style \glspl{element} with valid \gls{CSS}.
    By not requiring invalid \gls{CSS}, the library can easily be incorporated into existing projects.
    Additionally, \gls{CSS} tools can advantageously be used in tandem with the \gls{ELQ} syntax --- something that is not possible with related libraries that require invalid \gls{CSS}.
    ...
    Related libraries only support such feature by using invalid \gls{CSS}.
    By using related libraries that require invalid \gls{CSS}, the \gls{Bootstrap} framework would not be possible to alter with as few altered lines of code as achieved by \gls{ELQ} since the \gls{LESS} preprocessor would not understand the invalid \gls{CSS} syntax.
    Also, such related libraries impose a significant performance impact since the responsive \glspl{element} need to be evaluated at runtime in \gls{Bootstrap}.
    ...
    Related libraries have different advantages and limitations.
    ...
    Related libraries that are limited and not able to detect element resize events are then suitable as the performance penalty of doing so is avoided.
    Fortunately, since \gls{ELQ} is plugin based the same behavior as such related libraries can be achieved with plugins (or options) and may therefore still be the preferred library to use.
    ...
    A major difference to related libraries is that \gls{ELQ} is powered by a highly optimized subsystem for detecting \gls{element} resize events, which provides a significant speedup to related libraries that also detect \gls{element} resize events.
    The subsystem used by \gls{ELQ} has been shown to have the same or better performance as the other subsystems in FireFox and Safari.
    It has been shown that the optimized subsystem performs 32-fold (for the object-based solution) or 13-fold (for the scroll-based solution) faster than the subsystems used by related libraries in Chrome when preparing 500 \glspl{element} to be detectable.
    Additionally, some solutions used by related libraries suffer from issues that \gls{ELQ} does not as described in Section~\ref{sec:imp_erd}.
    \todo{Finish this.}
  ...
  \chapter{Conclusions}\label{ch:conclusions}
    ...
    \gls{ELQ} has been shown to have overall significantly better performance than related libraries.
    Some related libraries achieve the same performance as \gls{ELQ} in Safari, but not in other \glspl{browser}.
    In Chrome, the speedup is 32-fold when operating on 500 \glspl{element} compared to related libraries that use the object-based \gls{element} resize detection subsystem.
    ...
    Since \gls{ELQ} is released as open source, developers can now use element queries to create responsive modules as \gls{ELQ} is performant, compatible, flexible, powerful, and extensible.
    No other related library simultaneously provides all of these desirable properties.
  ...
  \chapter{Future work}\label{ch:future-work}
    ...
    This would be a key feature in order to further satisfy goal~\ref{itm:req_flexible}.

    \paragraph{Prolyfill}
    To satisfy goal~\ref{itm:req_prolyfill} an element queries prolyfill plugin could be created that would parse custom \gls{CSS} at runtime to simulate \gls{native} element queries.
    ...
    Different subsystems of \gls{ELQ} are forced to create their own \glslink{batch processing}{batch} processor, which is favorable in some cases.
    ...
    For instance, the element resizing detection subsystem and the \code{elq-breakpoints} plugin both use a \glslink{batch processing}{batch} processor to avoid \gls{layout thrashing}.
    However, since they process their \glslink{batch processing}{batches} independent from each other, the element resizing detection subsystem forces a layout before the plugin \glslink{batch processing}{batch} processor is invoked.
    ...
    Even though there are several issues with runtime parsing of CSS, it is probably beneficial in some cases.
    Recall from Section~\ref{sec:imp_erd} that CSS selectors may unintentionally style the injected \glspl{element} of the element resizing detection subsystem.
    By parsing CSS it would be possible to warn developers of such selectors.
    The feature could be valuable in the development phase of applications, but should probably be disabled in production due to performance and compatibility reasons.
    Developers that value true separation of style and \gls{HTML} could also benefit from parsing CSS, as the breakpoints of all \glspl{element} then could be deduced by considering the element queries in the CSS.
  ...
  \printbibheading
    ...
    \printbibliography[filter=papers,title={Articles},heading=subbibliography]
    \printbibliography[category=theses,title={Master's theses},heading=subbibliography]
    \printbibliography[category=w3cmaterial,title={Material of the W3C},heading=subbibliography]
    \printbibliography[category=relatedlibraries,title={Related Libraries},heading=subbibliography]
    \printbibliography[type=online,notcategory=relatedlibraries,notcategory=w3cmaterial,title={Online resources},heading=subbibliography]
    ...
    \chapter{Miscellaneous}
      \section{Full-scale Bootstrap Figures}
        ...
        \begin{figure}[htbp!]
          ...
          \caption{Double column documentation page powered by \gls{Bootstrap} showing the header section of the page. The \gls{viewport} is big enough to give each column sufficient space.}
