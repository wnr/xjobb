...
\begin{document}
  ...
  \chapter{Introduction}
    ...
    \section{Problem statement}
      By using \gls{CSS} \emph{media queries} developers can specify different style rules for different viewport sizes.
      ...
      Modular \gls{responsive} components should be able to react and change style depending on the size that the component has been given by the application, not the viewport size.
    ...
    \section{Objective}
      ...
      \begin{itemize}
        ...
        \item If a custom \gls{API} is developed, how would one make third-party modules (that uses media queries) work without demanding a rewrite of all third-party modules? \todo{Remove me?}
    ...
    \section{Significance}
      ...
      Without element queries, \gls{responsive} modules force the application to style them properly depending on the viewport sizes, which defeats the purpose of modules.
    ...
    \chapter{Web development}
      ...
      \section{Responsive web design}\label{sec:rwd}
        \begin{metatext}
          ...
          Since typically only desktop computers with monitors accessed \gls{web} sites they were designed for a minimum viewport size.
        ...
        With the release of \gls{CSS} \emph{media queries} new possibilities opened up.
        ...
        \begin{lstlisting}[caption={The above \gls{CSS} styles the body of the website blue if the viewport is less or equal to 600 pixels wide, and yellow otherwise.},captionpos=b]]
...
@media screen and (min-width: 601px) {
  body {
    background-color: yellow;
        ...
        \gls{RWD} refers to the approach of having a single site being \gls{responsive} to different media properties (mainly viewport size) at runtime to improve the user experience.
  ...
  \part{Theory}\label{part:theory}
    ...
    \chapter{Element queries}
      ...
      \Gls{responsive} \gls{web} design implements this by using media queries, applying conditional rules by the viewport size.
      ...
      Since the only way of applying conditional rules to elements today is by using media queries, all \gls{responsive} styles must be defined by the application and not by the modules (because only the application knows how much space each module will be given in the layout).
  ...
  font-size: 1em;
      ...
      \section{Possible solutions}
        ...
        \subsection{Viewport elements}\label{sec:viewport-elements}
          ...
          This means that a new \gls{HTML} tag will be crafted (perhaps named \code{viewport}) that defines separated viewports in the document.
          ...
          The reason that a new \gls{HTML} tag is proposed instead of a new \gls{CSS} property that defines the behavior is because the layout engine in the latter case needs to resolve the styles for all elements in order to know which are viewport elements.
          With a new \gls{HTML} tag, the layout engine can detect that that the element is a viewport before any resolving styles.
          ...
          The internal \glspl{API} that request non-layout information for the elements using element queries only need to make sure that the containing viewport element has been laid out before resolving the styles (which is a much better situation that the unrestricted version of element queries).
      ...
      \section{Native API design}
        ...
        As presented in subsection~\ref{sec:viewport-elements}, viewport elements solve many of the element query problems and are therefore assumed to be a part of a native \gls{API}.
        Since it would be favorable to be able to specify multiple conditional rule sets for the element queries (like media queries) the \gls{CSS} syntax may be of the form as presented in listing~\ref{code:mq-eq-syntax}.
