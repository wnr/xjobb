...
\begin{document}
  ...
  \chapter{Introduction}
    ...
    \todo[inline]{Renamed framework to library?}
    ...
    \section{Problem statement}
      ...
      The problem can be formulated as: \emph{Elements cannot specify conditional style rules depending on their own size, or the size of any other element}.
    ...
    \section{Significance}
      ...
      One of the biggest issues that keep web modules from being truly modular is that they cannot adapt to given sizes.
  ...
  \part{Background}
    ...
    \chapter{Browsers} 
      ...
      \section{The origin of the web}
        ...
        \subsection{The World Wide Web}\label{sec:www}
          ...
          As the protocol was simpler than \textsc{http} it was often preferred since it used less network resources.
    ...
    \chapter{Web development}
      ...
      \section{From documents to applications}
        ...
        To generate dynamic content on the server is sometimes referred to as \emph{server-side scripting}.
  ...
  \part{Theory}
    \chapter{Layout engines}
      ...
      \section{Constructing \textsc{DOM} trees}\label{sec:dom-tree}
        ...
        \textsc{dom} is an abbreviation of Document Object Model which is a convention for representing and interacting with object in \textsc{xml}-based documents (such as \textsc{html}).
      ...
      \section{Render trees}\label{sec:render-tree}
        ...
        Typically nodes in the \textsc{dom} tree map in a 1:1 relation to nodes in the render tree, but the relation can also be smaller or larger.
        ...
        For instance, a \textsc{dom} node element with the \code{display} property set to \code{none} will have the relation 1:0 (it will not be present in the render tree because it is not visible and will not affect the layout flow).
      ...
      \section{Style computation}\label{sec:style-computation}
        ...
        \end{itemize}
        The weighting process continues by calculating the \emph{specificity} of all rule set selectors, the higher the specificity the higher the rule set will be weightened.
      ...
      \section{The layout process}\label{sec:layout-process}
        ...
        The system will mark which renderers in the tree that need a layout, which avoids layout of unaffected elements.
      ...
      \section{Parallelization}\label{sec:parallel}
        ...
        On computers, browsers usually achieve parallelism by running each page context in parallel (each tab of the browser window is an own process).
    ...
    \chapter{Element queries}
      ...
      Although the typically the size of the container element is the most interesting element query with responsive web design, element queries as a concept does not need to be limited to that.
...
#foo:eq(min-width: 500px) #child { 
  ...
  font-size: 1em;
      ...
      \section{Native API design}
        ...
        As presented in subsection~\ref{sec:viewport-elements}, viewport elements solve many of the element query problems and are therefore assumed to be a part of a native \textsc{api}.
        Since it would be favorable to be able to specify multiple conditional rule sets for the element queries (like media queries) the \textsc{css} syntax may be of the form as presented in listing~\ref{code:mq-eq-syntax}.
...
... {
  ...
  \part{Third-party framework}
    ...
    \chapter{Framework design}
      ...
      First of all, a working name of the framework needs to be established --- the framework will from now on have the working name \textsc{elq}.
      This name will serve as a prefix for many of the framework \textsc{api}'s, and will be frequently used in the report from now on.

      \todo{Write more here or move the above text somewhere else?}
      ...
      \section{Technical goals}
        ...
        \begin{enumerate}
          \item \textbf{Features}
            \begin{enumerate}
              \item The framework needs to augment native element queries in the sense that styles are applied automatically on element resizes.
          ...
          \item \textbf{Ease of use}
            \begin{enumerate}
              \item Developers should not need to perform big rewrites of their modules or applications in order to use the framework.
              \item The \textsc{api}'s need to be natural for web developers, and may not seem alien.
              \item The framework should help identifiying cyclic rules and prevent them from causing infinite layouts.
            \end{enumerate}
          \item \textbf{Performance}
            \begin{enumerate}
          ...
          \item \textbf{Compatibility}
            \begin{enumerate}
              ...
              \item Dependencies and assumptions about the host application (including the development environment) should be kept low.
            ...
            \end{enumerate}
        \end{enumerate}
        It is possible to create a framework that conforms to all of the requirements but more features, automation and compatability decreases the performance.
        In the same way, too many advanced features may decrease the ease of use.
        Some feature may also decrease the compatability.
        It would be possible to create a framework that conforms to the requirements with some kind of balance between them.
        However, trying to conform to all of the requirements and trying to find a balance will result in a worse solution than a framework tailored for specific requirements.
        ...
        This way, developers are composing their own custom tailored framework for their specific use cases.
        ...
        For instance, the requirement~\ref{itm:req_prolyfill} is beneficial to satisfy with a plugin since the \textsc{api} for native element queries can only be guessed at this point of time, which will probably lead to frequent changes to the plugin.
        By separating it from the core framework (and the other plugins), only the developers that really desire the prolyfill behavior must handle the rapid \textsc{api} changes.
      ...
      \section{Architecture}
        \begin{metatext}
          ...
          The major subsystems of the core framework will be explained in depth. \todo{Should they be explained in depth here or in the implementation section? Maybe rewrite this sentence.}
        \end{metatext}

        To decide which parts of the framework that should be part of the core and which parts to be plugins is a difficult task.
        Once again, a balance needs to be found between ease of use, performance and extendability.
        ...
        \begin{figure}
          ...
          \tiny
            ...
            \node[mylabel, rotate=-90, text width=10em] (interpluginapilabel) at ($ (mirror.east) + (1.5,0) $) {Interplugin \textsc{api}};
            ...
            \draw[o] let \p1=(pubapibottom), \p2=(plugins.south) in ($ (plugins.south) + (-0.5,0) $) -| ($ (\x2,\y1) + (-0.5,0) $);
            \node[mylabel, right=4em of pubapibottom, text width=5em] (pubapilabel) {Public \textsc{api}};
            ...
            \node[mylabel, below=3em of pluginapi, text width=5em] (pluginapilabel) {Plugin \textsc{api}};
          ...
          \medskip
          \caption{
            The overall framework architecture, which shows how the framework is divided into two parts; the core system and the plugins. 
            The core is only accesible through the plugin \textsc{api}, and is not a part of the public \textsc{api}.
            Plugins may have interplugin \textsc{api}'s and dependencies.
            The bigger part of the public \textsc{api}'s is defined by the plugins.
          }
          \label{fig:elq_architecture}
        \end{figure}

        Figure~\ref{fig:elq_architecture} shows the overall architecture of the framework.
        The core consists of fundamental and general subsystems that are utilized by the plugins by the plugin \textsc{api}.
        Developers should never be programming against the core directly, and can therefore only be accessed by plugins through the hidden plugin \textsc{api}.
        ...
        It should be noted that plugins may provide \textsc{api}'s through the framework by registering methods, or by specifying custom syntax (such as markup or \textsc{css}) which is beyond the control of the framework.
        Plugins may also have interplugin \textsc{api}'s and dependencies, which also is beyond the control of the framework.
        The core system is supposed to have a slow development phase, with few changes and good backwards compatability.
        The plugins may be development in a high rate, with frequent changes to support the newest features.
        ...
        \begin{itemize}
          \item \textbf{Reporter:}
            ...
            Other options suchs as throwing exception on errors or warnings could also be set at framework-level.
            Also, the framework can make sure that all reports are standardized and can provide extra information such as which plugin reported what.
            To avoid code duplications, it also beneficial to have a centralized report system so the plugins do not need to perform the same compatability checks (such as checking if the browser actually supports console reporting).
          \item \textbf{Element resize detector:}
            Provides an interface for listening to element resize events.
            This system is fundamental to plugins fulfilling the ease of use requirements since it enables the framework to be aware of when elements change sizes, instead of the host application keeping track of when elements have changed sizes.
            This is presented in greater detail in section~\ref{sec:imp_erd}.
          ...
          \item \textbf{Cycle detector:}
            As shown in section~\ref{} cyclic rules need to be detected at runtime.
            When a plugin wish to update the size state of an element (which in turn applies the conditional styles for the state) it can ask this system if the element has cyclic rules.
            It keeps track of the size state of the elements, and decides if the rules of an element are cyclic by options such as state frequency.
            This is presented in greater detail in section~\ref{sec:imp_cycle_detector}.
          ...
          \item \textbf{Batch processor:}
            ...
            This is presented in greater detail in section~\ref{sec:imp_batch_processor}.
        ...
        \end{itemize}
        Some of the subsystems may be partially accessible by through the public \textsc{api} such as the plugin handler and the element resize detector.
        It should be noted that the presented core subsystems are not the only thing that the framework consists of --- they are simply the ones worth mentioning.
        \todo{Maybe rewrite this a bit?}

      \section{API design}
        \begin{metatext}
          This section will describe the \textsc{api} of the framework.
          First, the public \textsc{api} of the non-plugin part of the framework will be presented, followed by the plugin \textsc{api}.
          After that, all current plugins will be described in depth addressing their features and \textsc{api}'s.
        \end{metatext}

        Recall from section~\ref{} that the bigger part of the public \textsc{api} is provided by plugins.
        The role of the framework is to provide plugins with systems to be used for performing element query tasks, which is done through the plugin \textsc{api}.
        Three plugins will be developed; 

        \section{Public \textsc{api}}
        \section{Plugin \textsc{api}}
        \section{Plugins}
          \subsection{Breakpoints}
          \subsection{Mirror}
          \subsection{Prolyfill}
        ...
        \end{metatext}
          ...
          \begin{itemize}
            ...
            \item \textbf{Scroll events by overflowing elements}:
              By injecting multiple elements, into the target element, that have overflowing content scroll events will occur when the target element is resized.
            ...
            \item \textbf{Injecting frames}:
              ...
              Then a resize event handler can be attached to the frame that will emit a resize event for every target element resize.
          ...
          Each poll would force the layout queue to be flushed since the computed style of elements will need to be retrieved in order to know if elements have resized or not.
          The performance impact of polling the \textsc{dom} frequently is not desirable.
          Since the polling will be performed all the time, the overall page performance will be decreased even if the page is idle, which is undesirable especially for devices running on battery.
          This is not a great solution since reading the \textsc{dom} constantly can be affect the performance negatively. \todo{Should make quick test to see how much CPU it uses when polling frequently}
          ...
          Browsers will only need to perform extra work when preparing elements for being resize-detectable and when the elements actually resize.
          ...
          \begin{itemize}
            ...
            \item \textbf{JavaScript may break}:
              ...
              Also, code that replaced the content of elements (such as \code{element.InnerHTML = ...}) will undesirably remove the injected frame element of if used on the target element.
