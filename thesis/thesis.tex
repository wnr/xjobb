...
\newglossaryentry{layout engine}{
  name={layout engine},
  description={The part of the browser that handles parsing, laying out and rendering web content. Layout engines are often open source and browsers usually acts as a shell on top of a layout engine. Sometimes also called rendering engines}
}
...
@media screen and (min-width: 601px) {
  ...
  \part{Theory}\label{part:theory}
    \chapter{Layout engines}
      ...
      The \gls{browser} engine acts as a high level interface to the \gls{layout engine}, and is responsible for providing the \gls{layout engine} with \glspl{URI} that should be rendered with layout options.
      The main responsibility of the \gls{layout engine} is to render the current state of the fetched \gls{hypertext} document.
      Since documents may (and often do) change dynamically after parsetime, it is important to keep in mind that the job of the \gls{layout engine} is continuous, and is not a one time operation.
      ...
      \begin{table}[ht]\center
        ...
        \end{minipage}
        \caption{
          The major \glspl{layout engine} and their \glspl{browser} with market shares.
      ...
      \section{Parallelization}\label{sec:parallel}
        \begin{metatext}
          ...
          Parallelization is something \gls{layout engine} vendors are very interested in, and much research is being done about utilizing multiple cores to increase the speed of the engine.
    ...
    \chapter{Element queries}
      \begin{metatext}
        Now that a good understanding of \glspl{browser} (especially the \gls{layout engine}), \gls{responsive} design, and modular development has been acquired it is time to address element queries --- the solution to modular \gls{responsive} \gls{web} design.
      ...
      \section{Problems}
        ...
        \subsection{Performance}
          \begin{metatext}
            ...
            As shown in section~\ref{sec:parallel} \gls{browser} vendors are very interested in parallelizing their \glspl{browser} and especially the \glspl{layout engine} to increase the page performance.
