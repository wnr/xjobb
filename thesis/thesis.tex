...
doubleWidths(elements); // ~700 ms
        ...
        \begin{figure}
          \centering
          \includegraphics[width=\linewidth]{images/layout-thrashing-example-1-fixed}
          \caption{}
          \label{fig:layout-thrashing-example-1}
        \end{figure}

        \begin{lstlisting}[caption={Example of avoiding layout thrashing by batch processing reads and writes to the \gls{DOM}. The code reads and double the widths of 1000 elements in \textasciitilde7 ms.}, captionpos=b, label={code:thrashing-example-fixed}]
/* JS */
