...
\newglossaryentry{third-party}{
  name={third-party},
  description={Refers to \glspl{API} and systems implemented on top of the \gls{browser}, usually in \gls{JavaScript}. Such APIs and systems must be included by developers into the \gls{document}, and are usually not described by standards specifications}
}
...
\newglossaryentry{document}{
  name={document},
  description={Strictly defined as something that has an \gls{URI} and can return representations of the identified resource in response to HTTP requests. If otherwise states, document will refer to \gls{HTML} \gls{web} pages in this thesis. In \gls{JavaScript}, document refers to the \gls{DOM} root}
}
\newglossaryentry{XHTML}{
  name={XHTML},
  description={Well-formed XML-based markup language that extends or mirros \gls{HTML}. May be parsed by \gls{XML} parsers since it conforms to the stricter \gls{XML} syntax}
}
...
\begin{document}
  ...
  \part{Background}\label{part:background}
    ...
    \chapter{Browsers} 
      ...
      The \gls{web} is a system for accessing interlinked \gls{hypertext} \glspl{document} and other media such as images and videos.
      ...
      \section{The birth of the World Wide Web}\label{sec:www}
        ...
        In 1989 a proposal for a communication system that allowed interlinked \glspl{document} was submitted to the management at the \gls{CERN}.
    ...
    \chapter{Web development}
      ...
      \section{From documents to applications}
        ...
        Static content were written in \gls{HTML}, which could include hyperlinks to other \gls{hypertext} \glspl{document} or hypermedia.
        ...
        There is no way for the server to change the content that the client has received, unless the client requests another \gls{document}.
...
@media screen and (min-width: 601px) {
  ...
  \part{Theory}\label{part:theory}
    \chapter{Layout engines}
      ...
      Modern \glspl{browser} can display multiple pages at the same time (by using tabs, multiple windows or frames) where each page typically has an instance of the \gls{layout engine} in a separate process.
      ...
      \section{Constructing DOM trees}\label{sec:dom-tree}
        ...
        Each node of the \gls{document} is a \gls{DOM} object, and therefore the whole \gls{document} is converted to a tree structure of \gls{DOM} nodes which is referred to as the \gls{DOM} tree.
      ...
      \section{Render trees}\label{sec:render-tree}
        ...
        Some properties depend on the flow of the \gls{document}, and needs to be resolved through a layout.
