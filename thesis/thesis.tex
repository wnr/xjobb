      ...
      \section{Possible solutions}
        ...
        \subsection{Viewport elements}\label{sec:viewport-elements}
          \begin{metatext}
            ...
            The subsection is mainly based on \normalfont{\cite{w3c_eq_mail,ricg_irc_log,ricg_issue_viewport}}.
          \end{metatext}
          ...
          In order to avoid cyclic rules, the size of \code{viewport} \glspl{element} would not be dependent on their children, and element queries may only target the closest ancestor \code{viewport} of the targeted selector element.
          This way, cyclic rules can no longer occur due to size changes of children since the children may not alter the \gls{viewport} size.
          With such viewport element, the only way cyclic rules can occur is if the viewport element has rule sets that styles it differently depending on the size of itself (by using element queries).
          Since the behavior of some style properties of the \code{viewport} already need to be redefined (such as \code{display: inline;}) such style rules could also be handled specially for \code{viewport} \glspl{element}. \todo{Flesh this out a bit? Give example of when a cycle still can occur, and why it is valid to restyle the viewport with element queries (nested viewports for instance)}
          ...
          With a new \gls{HTML} tag, the \gls{layout engine} can detect that that the \gls{element} is a \gls{viewport} before resolving any styles.
          \todo[inline]{Write that viewport elements may only be part of selectors with combinators, cannot occur more than once and may not be the last in the selector.}
          \todo[inline]{Write why it is important to know viewport elements before resolving styles.}
