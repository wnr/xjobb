...
\begin{document}
  ...
  \chapter{Discussion and Conclusions}
    ...
    Unrestricted element queries imposes limitations to layout engines, and are therefore infeasible to implement.
    ...
    This design decision has proved to be very valuable when performing the altering of Bootstrap in Section~\ref{}.
    ...
    \todo[inline]{Philipp: you should try to summarize the performance advantage over other systems (e.g., "in some cases it's so-and-so-many \% faster thanks to our so-and-so-approach"). Maybe in the paragraph "Element resize detection"?}
    \todo[inline]{Philipp: add 1 or 2 sentences that summarize what ELQ provides, and what its benefits are over other approaches.}
    ...
    \section{Future work}
      ...
      In order to allow more advanced element query constellations, the mirror element could could be exteneded to target an arbitrary element (instead of just the nearest \code{elq-breakpoints} ancestor element).
      ...
      \todo[inline]{Philipp: Maybe you could insert named paragraphs that identify different areas for future work. That way section 7.3 would be easier to read.}
