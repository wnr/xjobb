...
@media screen and (min-width: 601px) {
  ...
  \part{Theory}\label{part:theory}
    \chapter{Layout engines}
      ...
      The \gls{browser} engine acts as a high level interface to the \gls{layout engine}, and is responsible for providing the \gls{layout engine} with \glspl{URI} that should be rendered.
      Additionally, browser engines usally provide layout engines with layout and rendering options such as user preferred font size, zoom, etc.
      ...
      \begin{enumerate}
        \item Fetch content (typically \gls{HTML} and \gls{CSS}) and parse it in order to construct a \gls{DOM} tree. \todo{Explain DOM before this?}
