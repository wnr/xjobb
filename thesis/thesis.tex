...
\begin{document}
  ...
  \begin{abstract}
    ...
    This is due to the limitation of CSS that elements can only be conditionally styled by the document root and device properties by using \gls{media queries}.
    ...
    As part of the empirical evaluation the Bootstrap framework has been altered to use element queries intead of \gls{media queries} by altering \textasciitilde50 out of \textasciitilde8500 lines of style code, which displays one of the advantages of the developed library.
  ...
  \chapter{Introduction}
    ...
    \section{Outline}
      ...
      \chref{ch:background} presents the background of this thesis and aims to provide sufficient information about \gls{web} development and layout engines to understand the problem and solution.
      In Section~\ref{sec:web-dev} the two \gls{web} development concepts \gls{responsive} \gls{web} design and modulairy are presented, and the Section ends describing the problem of \gls{responsive} modules.
      ...
      The Bootstrap framework provides many \gls{responsive} \gls{CSS} classes to make user interfaces \gls{responsive} by using \gls{media queries}.
      By altering Bootstrap to use element queries instead for the \gls{responsive} classes, some of the technical goals can be evaluated.
  ...
  \chapter{Background}\label{ch:background}
    ...
    \section{Web development}\label{sec:web-dev}
      ...
      \subsection{From documents to applications}\label{sec:doc-to-app}
        ...
        Additionally, as the \gls{web} platform was improved and users being discoraged by browsers from installing plugins due to security issues, the use of plugins decreased further.
      ...
      \subsection{The problem of responsive modules}\label{sec:rwd-modular-problem}
        ...
        This is due to \gls{media queries} only targeting the viewport, and not the actual width given to the instances.
        \begin{figure}[ht]
          \centering
          \begin{minipage}{.5\textwidth}
            \centering
    ...
    \section{Layout engines}\label{sec:layout-engines}
      ...
      \subsection{Layout thrashing and how to avoid it}\label{sec:layout-thrashing}
        ...
        When the computed style has been acquired, the width of the element is set to a new value, which is a \gls{DOM} mutation that will need to be synchronized with the \gls{render tree}.
  ...
  \chapter{Element queries}\label{ch:eq}
    ...
    \section{Definitions and Usages}\label{sec:eq-definitions}
      ...
      \Gls{media queries} and element queries are similar in the sense that they both enables developers to define conditional style rules that are applied by specified criterion \cite{w3c_css_mq}.
      The main difference is the type of criterion that can be used; in \gls{media queries} device and media criterion is used, while in element criterion is used in element queries.
      It can somewhat simplified be described as that \gls{media queries} target the document root and up (i.e., the viewport, browser, OS, device, input mechanisms, etc.) while element queries target the document content (i.e., elements of the document).

      \paragraph{Terminology}
      The entity that is evaluated against a query is called the \emph{target} of the query.
      For \gls{media queries}, the target is usually the device or viewport.
      ...
      According to the draft written by the \gls{RICG} the general use case is to conditionally style an element by an ancestor element's width \cite{ricg_draft} to allow \gls{responsive} \gls{web} design for reusable modules.
      Pages are usually designed to grow in height when content does not fit the viewport and therefore \gls{responsive} \gls{web} design usually targets the viewport width for layout breakpoints \cite{book_rwd,wiki_rwd,mjelde2014performance}.
      ...
      Since element queries are to mainly allow \gls{responsive} \gls{web} design for modules, the same is assumed to apply for element queries (that only the width of elements are of interest).
      Further, it is assumed that element queries mainly targets ancestor elements of the element to be conditionally styled, and therefore it is not crucial to support element queries on the own element width.
      This also implies that element queries that targets void elements are also not crucial, and hence are not regarded as a general use case.
    ...
    \section{Approaches to overcome the problems}\label{sec:eq-approaches}
      ...
      This is a powerful insight, since the general use case would be to write element queries against the width and have the \glspl{element} adapt their heights accordingly (including the viewport \gls{element}) (as described in Section~\ref{sec:eq-definitions}).
  ...
  \chapter{The element queries library}\label{ch:library}
    Now that sufficient knowledge about element queries and layout engines has been acquired it is time to present the actual realization of the solution to \gls{responsive} modules --- the JavaScript element queries library.
    ...
    \section{Details of subsystems}\label{sec:library-imp-details}
      ...
      \subsection{Element resize detection}\label{sec:imp_erd}
        ...
        According to the general use case described in Section~\ref{sec:eq-definitions}, a valid limitation is to only support element resize detection for non-void elements (i.e., elements that can contain content).
        ...
        \begin{enumerate}
          ...
          \item\label{itm:erd-approach-flow}
            ...
            This solution is similar to the scroll-based solution in the sense that it also injects elements that have overflowing content.
        ...
        While not stated officially by \cite{backalley}, the flow-based solution was probably rejected due to the lack of browser compatability.
        The flow-based solution is not presented in detail, due to the limited browser compatability and similarity with the scroll-based solution.
        \todo{Not really happy with this sentence.}
        The scroll-based solution was probably rejected due to issues with handling target elements with sides that are zero in length and also with the target element getting removed from the \gls{render tree} (e.g., by setting the \code{display} style property to \code{none}).
        ...
        The \gls{Blink} layout engine is able to do so, which results in exceptional performance as shown in Section~\ref{sec:eval-perf-erd}.
        ...
        However, it is still beneifical to batch process the algorithm for such layout engines since only pure layouts need to be performed (instead of having to recompute styles and synchronize the \gls{DOM} and \glspl{render tree} before each layout).        
        ...
        The \code{MutationObserver} \gls{API} defined in the W3C working draft of the DOM level 4 specification \cite{w3c_dom_4} may seem like a good candidate to use for detecting element resize events at a first glance.
        Although not yet standardized, the \gls{API} is implemented in modern browsers according to the working draft.
        By using the \gls{API} an element it is possible to observe attribute or subtree changes to an element.
        By observing the \code{style} attribute of an element and the content of it (if the element size depends on its children), it is possible to detect direct style changes to an element (i.g., changes to the style attribute by JavaScript).
        The main limitation is that it is not possible to observe the computed style state of an element.
        For instance, detecting that the width of an element has been set to 50 \% with JavaScript is possible (it would not have been possible to detect the style change if the width style was calculated by a CSS cascade).
        However, when the parent container change size the observer would not detect that the element width of the element has changed, as the style attribute width still is 50 \%.
        This could theoretically be resolved by also observing the parent of the element, and so on up to the root of the document.
        Such solution would probably end up observing most elements of a page, which might be a performance penalty, and it would still not be able to detect all element resize events.
  ...
  \chapter{Empirical Evaluation}\label{ch:eval}
    ...
    \section{ELQ Bootstrap}\label{sec:eval-bootstrap}
      The \gls{responsive} parts of Bootstrap 3.3.2 (the grid, containers, \gls{responsive} utility classes, forms, etc.) are built with \gls{media queries}, and therefore only behave as desired when the Bootstrap elements can use the full width of the viewport.
      It should be noted that no height breakpoints are being used by Bootstrap (as also identified by the general use case in Section~\ref{sec:eq-definitions}).
      This implies that modules that uses any \gls{responsive} Bootstrap classes inherently only behave as desired when the module can use the full width of the viewport, which means that such modules are not \gls{encapsulated} and therefore many of the benefits of modules disappear.
      ...
      It is shown in figure~\ref{fig:eval-bootstrap-mq-broken} that the original version of Bootstrap (that uses \gls{media queries}) cannot adapt to such layout changes.
      \begin{figure}[htbp]
        \centering
        \begin{minipage}{.5\textwidth}
          \centering
        ...
        \end{minipage}
        \caption{
          The \gls{responsive} classes of Bootstrap cannot adapt to layout changes as shown with the two-column \gls{CSS} documentation example.
          Both documentation instances uses \gls{media queries}, which undesireably styles both columns as if they each fill the whole viewport width.
          ...
          The pag only behaves as desired if the viewport width is large enough so that the width of each column is bigger than the largest \glslink{media queries}{media query} breakpoint, as shown in the right figure.
      ...
      In order to make Bootstrap behave as desired in any layouts the \gls{responsive} parts of the framework were modified to use element queries instead of \gls{media queries} by using \gls{ELQ}.
      ...
      The idea is to have all \gls{responsive} classes conditionally styled with element queries by the size of the nearest ancestor container element.
      This implies that all elements that has any \gls{responsive} classes will be converted to \code{elq-mirror} elements since they want to mirror the breakpoints of the nearest ancestor \code{elq-breakpoints} element (i.e., the container elements).
      Since container elements may be nested they will be both \code{elq-mirror} and \code{elq-breakpoints} elements.
      Recall from section~\ref{sec:plugin-mirror} that it is beneficial for \gls{responsive} elements to be \code{elq-mirror} elements in order to not limit the element usage to a specific a \gls{HTML} structure.

      \paragraph{Altering the LESS code}
      Bootstrap mainly uses three width numbers for \gls{responsive} breakpoints: 768, 970 and 1170 pixels.
      ...
      This is especially desired when keeping a \glslink{fork}{forked} project up to date with the original project, as fewer diverged lines implies a lowered risk of merge conflicts.
      ...
      Listing~\ref{code:bootstrap-init-js} presents an example of such JavaScript code that dynamically adds the required element attributes and initializes all \gls{responsive} elements.
      \begin{lstlisting}[gobble=8,label={code:bootstrap-init-js},caption={Example of JavaScript code that adds the required \gls{ELQ} attributes dynamically to all \gls{responsive} elements and initializes them.},captionpos=b]]
      ...
      The double column documentation page using \gls{ELQ} Bootstrap on the other hand enables all \gls{responsive} elements to style themselves according a parent container element.
      ...
      \begin{figure}[htb!]
        ...
        \end{minipage}
        \caption{
          The left image shows the original Bootstrap \gls{responsive} utilities classes matrix.
      ...
      \begin{figure}[htb!]
        ...
        \end{minipage}
        \caption{
          The left image shows the original Bootstrap \gls{responsive} grid.
    ...
    \section{Performance}\label{sec:eval-performance}\label{sec:eval-perf-erd}
      ...
      %\subsection{Element resize detection}\label{sec:eval-perf-erd}
        ...
        It achieves a 32-fold speedup compared to the object-based solution and a 13-fold speedup compared to the original scroll-based solution when preparing 500 elements for resize detection.
        The \gls{ELQ} solution also scales better, as more clearly shown in figure~\ref{fig:erd-solutions-regressed} that includes polynomial regression graphs for all three solutions.
        Both scroll-based solutions have the same memory footprint (i.e., too low for reliable measurements).
        \begin{figure}[ht!]
          \tiny
        ...
        The \gls{ELQ} scroll-based solution is preferred for Chrome, as the injection is 32-fold faster (when operating on 500 elements) than the object-based solution while the resize detection performance is the same for both solutions.
        ...
        \begin{table}[ht]\center
          ...
          \end{tabular}
          \caption{Performance of the two solutions in different layout engines when operating on 100 elements.}
  ...
  \chapter{Related work}\label{ch:related-work}
    There are numerous third-party element queries libraries, which this chapter aims to list and analyze.
    Since many of them share the same characteristics, they are classified and analyzed in groups by characteristics.

    Identified third-party libraries that directly or indirectly enables element queries are presented in table~\ref{table:approaches-classifications} along with a classification of each library.
    ...
    \begin{itemize}
      ...
      \item \textbf{Automatic}: Libraries that updates affected element queries on viewport and element resize events automatically.
      \item \textbf{Semi-automatic}: Libraries that partly updates affected element queries. Usually only viewport resize events are handled automatically.
    \end{itemize}
    ...
    The advantage of having the compilation step at server side is increased performance since the browser understands the \gls{CSS} directly when it has been retrieved.
    ...
    \begin{itemize}
      ...
      \item When parsed, the library needs to apply its logic to the parsed \gls{CSS} and produce valid \gls{CSS}. The produced \gls{CSS} needs to be applied to the document by mutating the \gls{DOM}.
    ...
    As shown in table~\ref{table:approaches-classifications}, most libraries simply listens to the viewport resize event, which may be enough for static pages.
    ...
    \todo{Write why again?}
    Library \cite{eq_imp_gss} evaluates all elements on \gls{DOM} mutations, using the \code{MutationObserver} \gls{API} as discussed in Section~\ref{sec:imp_erd}, which does not detect all element resize events.
    \todo{Write more about GSS, how it uses constraints CSS, bad browser support, that it is fairly static, quote from github, etc.}
  ...
  \chapter{Discussion and Conclusions}\label{ch:discussion-conclusions}
    ...
    As \gls{web} applications are transitioning from being server generated to entirely living on the client, it is important to write modular components for various positive effects.
    ...
    Browsers behaves a bit different regarding the speculative rendering, but at least \gls{Blink}, \gls{Gecko} and \gls{WebKit} generally renders the page before all external scripts have been executed.
    ...
    ELQ and its plugins provides a low level API for building \gls{responsive} design frameworks and applications by enablind the user to conditionally style elements with valid CSS.
    ...
    The mirror plugin of ELQ enables developers to overcome some CSS limitations, which has been proven to be important when writing \gls{responsive} modules that are not limited to a predefined \gls{HTML} structure (as shown in Section~\ref{sec:eval-bootstrap}).
    ...
    The subsystem used by ELQ has been showed to have the same or better performance as the other subsystems in FireFox and Safari.
    It has been shown that the optimized subsystem performs 32-fold (for the object-based solution) or 13-fold (for the scroll-based solution) faster than the subsystems used by related work in Chrome when preparing 500 elements to be detectable.
    ...
    \section{Future work}
      ...
      \Gls{media queries} are at the moment the most performant way of applying conditional styles by viewport size, which possibly is the only factor to static page layout.
  ...
  \addappheadtotoc
    ...
    \chapter{Miscellaneous}
      ...
      \section{Full-scale figures}
        \subsection{Bootstrap}
          ...
          \begin{figure}[htbp!]
            ...
            \caption{Double column documentation page powered by Bootstrap showing the \gls{responsive} classes matrix section of the page.}
            \label{fig:appendix-bootstrap-mq-matrix-small}
          \end{figure}
          \begin{figure}[htbp!]
            \centering
            \includegraphics[width=0.9\linewidth]{images/bootstrap-eq-matrix}
            \caption{Double column documentation page powered by \gls{ELQ} Bootstrap showing the \gls{responsive} classes matrix section of the page.}
      ...
      \section{Layout engine market share statistics}\label{sec:layout_engines_market_share}
        ...
        StatCounter shows that 37\% of the Opera users are using Opera Mini (their mobile \gls{browser}), which does not use the \gls{Blink} engine (it uses Opera's own Presto \gls{layout engine} which will be ignored).
