...
\begin{document}
  ...
  \chapter{Background}\label{ch:background}
    ...
    \section{Layout engines}\label{sec:layout-engines}
      ...
      \subsection{Layout thrashing and how to avoid it}\label{sec:layout-thrashing}
        ...
        While it is algorithmically inefficient to iterate over the same collection twice --- the \glslink{batch processing}{batched} approach executes \textasciitilde100-fold faster than the original version.
  ...
  \chapter{Element queries}\label{ch:eq}
    ...
    Since no syntax has been defined yet for element queries, pseudo-syntax is presented in this section to be used throughout this thesis.
    ...
    \section{Definitions and Usages}\label{sec:eq-definitions}
      ...
      Recall from the \gls{CSS} selectors level 3 specification \cite{w3c_css_selectors} that pseudo-classes permit selection based on extended element information, and is often described as performing selection by the state of the \gls{element}.
    ...
    \section{Problems}\label{sec:eq-problems}
      ...
      \subsection{Performance}
        ...
        Unfortunately, this means that if there is any matching element query selector at page load, two layout passes always need to be performed.
  ...
  \chapter{The element queries library}\label{ch:library}
    ...
    \section{Architecture}\label{sec:arcithecture}
      ...
      \begin{itemize}
        \item \textbf{Reporter:}
          ...
          To avoid code duplication it is also beneficial to have a centralized reporting system so plugins do not need to perform the same compatibility checks (such as checking if the \gls{browser} actually supports console reporting).
    ...
    \section{API design}\label{sec:elq-api}
      \subsection{Public API}\label{sec:public-api}
        ...
        When called, it will invoke the \code{start} methods on all registered plugins that give them the opportunity to initiate the \glspl{element} according to their own mechanisms.
  ...
  \chapter{Empirical Evaluation}\label{ch:eval}
    ...
    Key subsystems are evaluated independently so that they can be compared to similar subsystems of related libraries.
