...
\begin{document}
  ...
  \chapter{ELQ --- The element queries library}\label{chp:library}
    ...
    \section{API design}\label{sec:elq-api}
      \subsection{Public API}\label{sec:public-api}
        ...
        It also exposes methods that are called by \gls{ELQ} in order to retrieve the plugin name, version and compatibility.
        When \code{use} is called, it creates a plugin instance by invoking the \code{make} factory function and returns the created instance so that the user may store the reference to the plugin instance.
    ...
    \section{Interesting implementation details}\label{sec:library-imp-details}
      ...
      \subsection{Element resize detection}\label{sec:imp_erd}
        ...
        Solution \ref{itm:erd-approach-scroll} and \ref{itm:erd-approach-frame} have similar characteristics and were both presented by \cite{backalley}.
  ...
  \chapter{Discussion and Conclusions}
    ...
    The problem was shown in Section~\ref{sec:eq} to be solved by element queries.
    ...
    Element queries has been identified as a solution to problem of having responsive modules.
    Native unrestricted element queries limits layout engines and therefore imposes problems to implementors.
    A draft
    With element queries comes 
    ...
    \section{Future work}
      ...
      \todo[inline]{elq-breakpoints performs an extra layout that could be incorporated into the erd system if really really optimized.}
      \todo[inline]{Investigate use cases and decide if there are things that can not be solved with ELQ, and in that case add such features.}
      ...
      \todo[inline]{Research if statically generated MQ can be used for static pages.}
      \todo[inline]{Let mirror target an arbitrary element}
      \todo[inline]{Instead of having independent batch processors it might be better to have a global batch processor that has phases (read, mutate, force) that every system can add functions to --- in order to avoid intersystem layout thrashing.}
