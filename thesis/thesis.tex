...
\subtitle{Allowing modular responsive web components to respond to custom criteria instead of only the viewport size by implementing an element queries JavaScript library}
...
\begin{document}
  ...
  \chapter{Empirical Evaluation}\label{ch:eval}
    ...
    \section{Performance}\label{sec:eval-performance}\label{sec:eval-perf-erd}
      ...
      %\subsection{Element resizing detection}\label{sec:eval-perf-erd}
        ...
        Since the element resizing detection subsystem performs the heaviest tasks, only the performance of that subsystem is evaluated in detail.
        ...
        \begin{figure}[ht!]
          ...
          \begin{center}
            ...
            \begin{minipage}[t]{.5\textwidth}
              ...
              \end{tikzpicture}
            \end{minipage}
          \caption{The injection performance of the optimized \gls{ELQ} scroll-based element resizing detection solution. The left graph shows the injection time of the \gls{ELQ} scroll-based solution. The right graph shows all three solutions for comparison. The heap memory usage graph has been omitted as the memory usage for the scroll-based solutions are near constant.}
  ...
  \chapter{Related work}\label{ch:related-work}
    ...
    According to the evaluation in Section~\ref{sec:eval-performance}, all three libraries are less performant than the element resizing detection system used in \gls{ELQ}.
  ...
  \chapter{Discussion}\label{ch:discussion}
    ...
    The most imminent drawback is perhaps that element queries are not evaluated when \glspl{layout engine} render the page for the first time, as \glspl{layout engine} usually renders a speculative state of the page before all content has been downloaded and parsed (e.g., external scripts).
