...
\newglossaryentry{viewport}{
  name={viewport},
  description={The outer frame that defines the visible area of the document. Usually defined by the \gls{browser} window, but may be restricted by other factors such as frames}
}
...
\newglossaryentry{native}{
  name={native},
  description={Refers to \glspl{API} and systems implemented and provided by \glspl{browser}. Such APIs and systems are often described by standards specifications}
}
\newglossaryentry{third-party}{
  name={third-party},
  description={Refers to \glspl{API} and systems implemented on top of the \gls{browser}, usually in \gls{JavaScript}. Such APIs and systems must be included by developers into the document, and are usually not described by standards specifications}
}
...
\newglossaryentry{JavaScript}{
  name={JavaScript},
  description={\Gls{native} \gls{browser} script language. Web documents often include JavaScript to make the documents more dynamic and interactable}
}
\newglossaryentry{browser}{
  name={browser},
  description={Client application for navigating the \gls{WWW} and displaying \gls{web} pages}
}
  ...
  \chapter{Introduction}
    ...
    \section{Problem statement}
      ...
      \begin{itemize}
        ...
        The research front of \glspl{browser} have shown successful ways of parallelising the layout engine.
  ...
  \part{Background}\label{part:background}
    ...
    \chapter{Browsers} 
      \begin{metatext}
        ...
        This chapter will give a brief history of \glspl{browser} and how the \gls{web} transitioned from handling science documents to commercial applications.
      ...
      \section{The birth of the World Wide Web}\label{sec:www}
        ...
        The idea was to allow links to other documents embedded in text documents, directly accesible for users.
        ...
        It continues to explain that interlinked resources does not need to be limited to text documents since multimedia such as images and videos can also be interlinked which would similarly be hypermedia.
        The concept of \glspl{browser} is described, with a client-server model the \gls{browser} would fetch the \gls{hypertext} documents, parse them and handle the fetching of all media linked in the \gls{hypertext}.

        In 1990, \gls{HTTP} and \gls{HTML} implemented.
        A \gls{browser} and a \gls{web} server had also been created and the \gls{web} was born.
        ...
        At this time \gls{W3C} was founded with support from the \gls{DARPA} and the European Commission.
        ...
        The listed documents of a server could then be presented as \gls{hypertext} menus to the client (much like a \gls{web} \gls{browser}).
      ...
      \section{The history of browsers}
        ...
        \begin{metatext}
          ...
          To fulfill the vision that users would be able to skip to the linked documents ``with a click of the mouse'' users needed a client to handle the fetching and displaying of the \gls{hypertext} documents, hence the need for \glspl{browser} were apparent.
        ...
        The \gls{WebKit} layout engine was made fully open source in 2005.
    ...
    \chapter{Web development}
      \begin{metatext}
        ...
        This chapter will describe the transition from \glspl{browser} rendering simple documents to being hosts for complex applications.
      ...
      \section{From documents to applications}
        \begin{metatext}
          This section will describe the transition from \glspl{browser} rendering simple documents to being hosts for complex applications.
        ...
        As described in section~\ref{sec:www}, \glspl{browser} initially were applications that displayed \gls{hypertext} documents with the ability to fetch linked documents in an user friendly way.
        ...
        In 1996 \gls{W3C} officially recommended \gls{CSS}, which came to be the preferred way of styling \gls{web} content.
      ...
      \section{Responsive web design}\label{sec:rwd}
        ...
        When a \gls{browser} requests a resource, an \emph{agent string} is usually sent with the request to identify what kind of \gls{browser} the user is using.
        ...
        First, developers now have two versions of a site to maintain and develop in parallel.
        ...
        See listing~\ref{code:mq-example} for an example of how \gls{media queries} can be used to style \glspl{element} differently by the viewport size.
  ...
  body {
    background-color: yellow;
      ...
      \section{Modularity}\label{sec:modularity}
        \begin{metatext}
          As the web was a platform for \gls{hypertext} documents that quickly transitioned to serving complex applications, few techniques existed for writing modular code.
        ...
        It is then possible to reason about and change the functionality locally inside the boundary of the module.
  ...
  \part{Theory}\label{part:theory}
    \chapter{Layout engines}
      \begin{metatext}
        ...
        Only the layout engines of the \gls{browser} will be of importance, since element queries do not affect any other \gls{browser} subsystem.
      ...
      \end{figure}
      It is shown that the layout engine is located between the \emph{\gls{browser} engine} system and the network, \gls{JavaScript}, \gls{XML} parsing and display backend.
      The \gls{browser} engine acts as a high level interface to the layout engine, and is responsible for providing the layout engine with \glspl{URI} that should be rendered with layout options.
      ...
      Modern \glspl{browser} can display multiple pages at the same time (by using tabs or multiple windows) where each page typically has an instance of the layout engine in a separate process.
      There are four layout engines that the major \glspl{browser} use, as presented in table~\ref{table:layout_engines}.
      ...
      \begin{table}[ht]\center
        ...
        \end{minipage}
        \caption{
          The major layout engines and their \glspl{browser} with market shares.
    ...
    \chapter{Element queries}
      \begin{metatext}
        Now that a good understanding of \glspl{browser} (especially the layout engine), \gls{responsive} design, and modular development has been acquired it is time to address element queries --- the solution to modular \gls{responsive} \gls{web} design.
      ...
      \section{Problems}
        ...
        \subsection{Performance}
          \begin{metatext}
            ...
            As shown in section~\ref{sec:parallel} \gls{browser} vendors are very interested in parallelizing their \glspl{browser} and especially the layout engines to increase the page performance.
  ...
  \addappheadtotoc
    \chapter{Resources}
      ...
      \section{Layout engine market share statistics}\label{sec:layout_engines_market_share}
        ...
        Since the graph only display \gls{browser} market share and not layout engine, it is needed to further divide the \glspl{browser} into layout engine percentages.
        ...
        StatCounter shows that 37\% of the Opera users are using Opera Mini (their mobile \gls{browser}), which does not use the Blink engine (it uses Opera's own Presto layout engine which will be ignored).
      ...
      \section{Usage share of browser versions}
        ...
        See figure~\ref{} for the adoption rate of new versions of Chrome for an example of how evergreen \glspl{browser} keep their users updated.
