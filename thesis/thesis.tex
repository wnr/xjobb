...
%         \todo{Continue this if possible?}
  \part{Third-party framework}\label{part:framework}
    ...
    \chapter{Analysis of approaches}\label{ch:current_approaches}
      ...
      It should be noted that \cite{eq_imp_gss} has been left out deliberately, as their approach will be analyzed by itself in section~\ref{}.
      \begin{table}[ht]\center
        \tiny
        \begin{tabular}[t]{ l p{3cm} l l l p{3cm} }
          ...
          \cite{eq_imp_localised-css} &         Localised CSS &                               Invalid &   Automatic &       Dynamic &   Runtime parsing of CSS. Unfinished. Polling. \\
          ...
          \cite{eq_imp_prollyfill-min-width} & TODO & & &  &
        \end{tabular}
      ...
      % \cite{eq_imp_css-element-queries}   Valid sytnax.   Automatic. Dynamic. Parses CSS.
      % \cite{eq_imp_prollyfill-min-width} TODO:
      % \cite{eq_imp_gss}
      
      The approaches \cite{eq_imp_magichtml,eq_imp_eqcss,eq_imp_localised-css,eq_imp_gss} have in common that they require developers to write cunstom (invalid) \gls{CSS}.
      ...
      \begin{itemize}
        ...
        \item
          The \gls{API} and parser will need to be kept up to date with \gls{CSS} standards in order to make sure new features of \gls{CSS} are supported and that they are not conflicting with the custom \gls{API}.
        \item
