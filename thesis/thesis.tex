  ...
  \part{Theory}
    \chapter{Layout engines}
      ...
      \section{Constructing \textsc{DOM} trees}\label{sec:dom-tree}
        ...
        Parsing of \textsc{html} is not an easy task because the specification of \textsc{html} states that it must be forgiving (i.e. accepting malformated markup) \todo{Is it true that the sepc says that malformated html must be accepted. http://kmcallister.github.io/papers/2015-servo-experience-report-draft1.pdf seems to say otherwise.}.
      ...
      \section{Parallelization}
        \begin{metatext}
          ...
          This sections is mainly based on \normalfont{\cite{zoomm}\cite{parallelizing_the_web_browser}}.
        \end{metatext}

        As web applications grow bigger and more demanding, browser continously needs to improve the performance on all levels.
        ...
        This approach is appealing because it utilizes the cores of the machine by still having all subsystems run sequentally for each page, and improves the overall performance of the browser.
        However, this approach is not enough.
        The page performance is not improved if only one page is present (the user does not have multiple tabs open).
        ...
        It has been shown that the performance of mobile browsers are \textsc{cpu} bound contrary to the common belief that they are network bandwidth bound.
